...
@LastEditTime: 2023-04-18 15:42:03
...
class FrameBaseHandler(BaseApiHandler):
    ...
    def __get_encrypt_appkey(self, json_params):
        """
        :Description: 获取加解密appkey,当有参数app_id时，优先取app_id，然后在是source_app_id
        :param json_params: json参数字典
        :last_editors: HuangJianYi
        """
        app_key = None
        if json_params:
            app_key = app_key if app_key else json_params.get("app_id", "")
            app_key = json_params.get("source_app_id", "")
        if not app_key:
            app_key = app_key if app_key else self.get_argument("app_id", "", strip=True)
    ...
    def _convert_request_params(self):
        ...
        if self.is_api_encrypt() == True:
            ...
            app_id = self.__get_encrypt_appkey(json_params)
            if not app_id:
