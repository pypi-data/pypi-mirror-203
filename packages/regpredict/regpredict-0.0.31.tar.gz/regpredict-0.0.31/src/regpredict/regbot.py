...
class Regbot:
  ...
  @classmethod  
  def prepareInput(cls,vol,rsi_5,rsi_15,sma_5,sma_7,sma_25,vc_grad):
    #avr = closing/(opening + closing)
    #bvr = opening/(opening + closing)
    try:
      testdata = np.array([[vol,rsi_5,rsi_15,sma_5,sma_7,sma_25,vc_grad]])
  ...
  @classmethod
  def buySignalGenerator(cls,vol,rsi_5,rsi_15,sma_5,sma_7,sma_25,vc_grad):
    scalledInput = cls.prepareInput(vol,rsi_5,rsi_15,sma_5,sma_7,sma_25,vc_grad)
    try:
...
def signal(vol,rsi_5,rsi_15,sma_5,sma_7,sma_25,vc_grad):
  try:
    return Regbot.buySignalGenerator(vol,rsi_5,rsi_15,sma_5,sma_7,sma_25,vc_grad)
  except Exception as e:
    return {
      'Error': e
    }
