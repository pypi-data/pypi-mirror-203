...
def _setup():
    if not get_config('user_token'):
        set_config("user_token", 'dummy')
    if not get_config('current_project'):
        while True:
            try:
                path = prompt("Which directory would you like to use with Navajo?\n> ",
...
def converse():
    ...
    while True:
        try:
            query = prompt(
                HTML("<ansimagenta>navajo> </ansimagenta>"),
                history=history,
...
def start_server():
    daemon = multiprocessing.Process(target=server.main, daemon=True)
    daemon.start()

def main():
    parser = argparse.ArgumentParser(description='navajo')
