...
class CompatAnalyzerTC(CubicWebTC):
    ...
    def test_rules(self):
        with self.admin_access.cnx() as cnx:
            ...
            cnx.commit()
            self.assertFailingRules(doctor, ['SEDA 2.1', 'SEDA 1.0', 'SEDA 0.2', 'simplified'])

            unit, unit_alt, unit_alt_seq = testutils.create_archive_unit(transfer)
            transfer.cw_clear_all_caches()
            cnx.commit()
            self.assertFailingRules(doctor, ['SEDA 2.1', 'SEDA 0.2', 'simplified'],
            ...
            cnx.commit()
            self.assertFailingRules(doctor, ['SEDA 2.1'],
    ...
    def test_custodial_history(self):
        with self.admin_access.cnx() as cnx:
            ...
            unit_alt_seq.cw_clear_all_caches()
            cnx.commit()
            self.assertFailingRules(doctor, ['SEDA 2.1', 'SEDA 1.0', 'simplified'],
