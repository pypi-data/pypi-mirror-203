...
from databricks.feature_store.unity_catalog_client import FunctionInfo as UCFunctionInfo
from databricks.feature_store.unity_catalog_client import UnityCatalogClient
...
class TrainingScoringClient:
    def __init__(
        ...
        self._unity_catalog_client = unity_catalog_client
    ...
    def create_training_set(
        ...
        # TODO (ML-29793): Introduce relevant validations for FeatureLookup, FeatureFunction ordering
        ...
        # TODO (ML-29647): Validate label_names, exclude_columns uniqueness
        ...
        uc_function_infos = {
            odci.udf_name: self._unity_catalog_client.get_function(odci.udf_name)
            for odci in on_demand_column_infos
        }
        # Validate UDFs are Python and no larger than 100 KB.
        for udf_name, uc_function_info in uc_function_infos.items():
            if uc_function_info.external_language != UCFunctionInfo.PYTHON:
                raise ValueError(
                    f"FeatureFunction UDF '{udf_name}' is not a Python UDF. Only Python UDFs are supported."
                )
            if len(uc_function_info.routine_definition) > 100000:
                raise ValueError(
                    f"FeatureFunction UDF '{udf_name}' is too large to use in on-demand computation. "
                    f"Use UDFs 100KB or smaller in size."
                )
        # Validate FeatureFunction input bindings and UDF parameters match
        for odci in on_demand_column_infos:
            udf_input_params = [
                p.name for p in uc_function_infos[odci.udf_name].input_params
            ]
            if odci.input_bindings.keys() != set(udf_input_params):
                raise ValueError(
                    f"FeatureFunction input bindings {odci.input_bindings} did not match "
                    f"UDF '{odci.udf_name}' parameters {udf_input_params}."
                )
        ...
        # TODO (ML-29793): Determine if reordering of FeatureColumnInfos, OnDemandColumnInfos are required.
        column_infos = [
        ...
        # TODO (ML-30198): Use actual md5 or function_id once implemented
        function_infos = [
            FunctionInfo(udf_name=udf_name, md5=udf_name)
        ...
        # TODO (ML-29843): Make instrumentation safe when FeatureLookups DNE.
        #  Add instrumentation for FeatureFunctions and unit tests.
    ...
    def score_batch(self, model_uri: str, df: DataFrame, result_type: str) -> DataFrame:
        ...
        utils_common.validate_strings_unique(
            [col for col in df.columns if col in required_cols],
            "Columns required for scoring or looking up features cannot be duplicates. Found duplicate columns {}.",
        )
