...
class Adapter(logging.LoggerAdapter):
    ...
    def process(self, msg: str, kwargs: dict) -> (str, dict):
        ...
        kwargs['_ctx'] = ctx['correlation_id'] if ctx else ''
        return msg, kwargs
...
class TextFormatter(FormatterInterface):
    ...
    default_log_fmt = '%(asctime)s | %(levelname)5s | %(_ctx)s | %(name)s | %(message)s | %(_data)s'

    def __init__(
        self, *args, colored_mode: bool = True, datefmt: str = default_date_fmt, fmt: str = default_log_fmt, **kws
    ):
        """Initialize.
