...
class JSONRPCServer(ContextableService, AbstractRPCCompatible):
    ...
    async def _execute_request(
        ...
        _log_req = request if self.app.debug else dict(id=request.id, method=request.method, params='...')
        ...
        try:
            ...
            result = RequestTimeout(id=id_)
            _error = True
            _exc = None
        except APIException as exc:
            result = RPCError.from_api_exception(id_, exc, debug=self._debug)
            _error = True
            _exc = exc
        except Exception as exc:
            result = InternalError(id=id_, base_exc=exc, debug=self._debug)
            _error = True
            _exc = exc
        if _error:
            self.logger.info('rpc error', request=request, exc_info=_exc)
        else:
            _result = result.result if self.app.debug else '...'
