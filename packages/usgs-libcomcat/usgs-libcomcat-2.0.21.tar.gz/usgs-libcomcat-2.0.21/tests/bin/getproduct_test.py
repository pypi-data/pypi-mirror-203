...
def test_phases():
    ...
    try:
        cmd = (
            "getproduct -s 2012-09-05T13:42:07 -e 2012-09-05T15:42:07 -b"
    ...
    try:
        cmd = (
            'getproduct shakemap "grid.xml" -d %s -b 163.213 -178.945'
    ...
    try:
        cmd = (
            'getproduct shakemap "grid.xml" -d %s -r -36.514 177.933  5 -s '
...
def test_country():
    ...
    try:
        cmd = (
            'getproduct shakemap "grid.xml" -d %s -s '
            "2013-02-16T04:00:00 -e 2013-02-16T06:00:00 --country abc" % tmpdir
        )
        res, stdout, stderr = get_command_output(cmd)

        cmp = (
            "abc is not a valid ISO 3166 country code. "
    ...
    try:
        cmd = (
            "getproduct shakemap -d %s -s 2016-10-21T04:07:22 -e "
...
def test_scenario():
    ...
    try:
        fmt = (
            "getproduct shakemap-scenario cont_mi.json -i "
        ...
        if not os.path.isfile(outfile):
            fmt = (
                "getproduct command %s failed to create "
