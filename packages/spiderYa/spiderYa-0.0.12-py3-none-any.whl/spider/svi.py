...
def svi_nnSVG(idata, work_dir, overwrite=False):
    try:
        ...
        if (overwrite) | (not exists( f'{work_dir}nnSVG.csv')):
            t0=time.time()
            os.system(str(f'/bin/bash -c "source /etc/profile;module load GCC/11.2.0 OpenMPI/4.1.1 R/4.2.0 Anaconda3/2022.05 R-bundle-Bioconductor/3.15-R-4.2.0;R -f ./R_script/run_nnSVG.R {count_f} {meta_f} {work_dir}"'))
...
def svi_trendsceek(idata, work_dir, overwrite=False):
    try:
        ...
        if (overwrite) | (not exists( f'{work_dir}trendsceek.csv')):
            os.system(str(f'/bin/bash -c "source /etc/profile;module load GCC/11.2.0 OpenMPI/4.1.1 R/4.2.0 Anaconda3/2022.05 R-bundle-Bioconductor/3.15-R-4.2.0;R -f ./R_script/run_trendsceek.R {count_f} {meta_f} {work_dir}"'))
...
def svi_SPARKX(idata, work_dir, overwrite=False):
    try:
        ...
        if (overwrite) | (not exists( f'{work_dir}SPARKX.csv')):
            t0=time.time()
            os.system(str(f'/bin/bash -c "source /etc/profile;module load GCC/11.2.0 OpenMPI/4.1.1 R/4.2.0 Anaconda3/2022.05 R-bundle-Bioconductor/3.15-R-4.2.0;R -f ./R_script/run_SPARKX.R {count_f} {meta_f} {work_dir}"'))
