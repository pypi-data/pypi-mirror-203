...
class Loader:
    ...
    def zero_worker_iter(self):
        raw_tensor_set = set(self.raw_tensors) - set(self.json_tensors) - set(self.list_tensors)
        for batch in self.dataloader:
            for sample in batch:
                for tensor in self.pil_compressed_tensors:
                    if isinstance(sample[tensor], (list, tuple)):
                        sample[tensor] = list(Image.open(io.BytesIO(t)) for t in sample[tensor])
                    else:
        ...
        while 1:
            ...
            if i < len(dataloader):
                ...
                if self.pil_compressed_tensors:
                    all_bts, batch = combine_compressed_bytes(batch, self.pil_compressed_tensors, self.json_tensors, self.list_tensors)
                else:
                    all_bts = None
                batch = (all_bts, batch)
...
def early_transform_collate(inp):
    data_in_queue, data_out_queue, transform_fn, collate_fn, upcast, pil_compressed_tensors, json_tensors, list_tensors, raw_tensors = inp
    raw_tensor_set = set(raw_tensors) - set(json_tensors) - set(list_tensors)  # tensors to be returned as bytes
    while 1:
        try:
            ...
            if all_bts is not None:
                batch = bytes_to_batch(batch, pil_compressed_tensors, json_tensors, list_tensors, all_bts)
    ...
    for sample in batch:
        for tensor in all_byte_tensors:
            ...
            if isinstance(sample[tensor], tuple):
                ...
                sample[tensor] = [decompress_fn(io.BytesIO(data_bytes[sb:eb])) for sb, eb in sb_eb_list]
            else:
