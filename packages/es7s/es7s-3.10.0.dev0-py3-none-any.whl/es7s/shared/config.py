...
def get_app_config_yaml(name: str) -> dict|list:
    f = pkg_resources.resource_stream(APP_NAME, os.path.join(RESOURCE_DIR, f'{name}.yml'))
    return yaml.safe_load(f.read())


def get_default_config_filepath() -> str:
    return pkg_resources.resource_filename(APP_NAME, os.path.join(RESOURCE_DIR, 'es7s.conf.d'))
