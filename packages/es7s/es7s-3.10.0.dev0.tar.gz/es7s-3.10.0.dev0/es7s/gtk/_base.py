...
class IndicatorUnboundState:
    ...
    def log(self):
        get_logger().trace(repr(self), "State", out_sanitized=True)
...
class _ExternalBoolState(_BoolState):
    callback: t.Callable[[_State, th.Event], None] = None
    ext: th.Event = None

    def click(self):
        if self.callback:
            self.callback(self, self.ext)
...
class _BaseIndicator(ShutdownableThread, t.Generic[DT], ABC):
    ...
    def _render_result(
        ...
        if icon:
            get_logger().trace(icon, "SET Icon", out_sanitized=True)
    ...
    def _set(self, label: str, guide: str | None, status: AppIndicator.IndicatorStatus):
        if self._hidden:
            return
        get_logger().trace(label, "SET Label", out_sanitized=True)
        get_logger().trace(status.value_name, "SET Status", out_sanitized=True)
