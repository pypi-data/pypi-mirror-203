...
class DataH:
    ...
    def download(self,symbol_list=None,start_date='2022-01-01',end_date='2022-01-30',just_data=False,fill=False,**kwargs):       
        ...
        if fill:
            date_set = sorted(set(data[self.date_col]))
            data = data.groupby(self.symbol_col).apply(lambda x:x.drop(self.symbol_col,axis=1).set_index(self.date_col).reindex(date_set).ffill().reset_index()).reset_index(level=0)
...
def download_yahoo(symbol_list,start_date,end_date,**kwargs):
    try:
        ...
        logging.error('please install yfinance first')
        return 
...
def download_local(symbol_list=None,start_date=None,end_date=None,**kwargs):
    from singletrader.datasdk.sql.dataapi import get_price
    data = get_price(start_date=start_date,end_date=end_date,universe=symbol_list)
    data = data.reset_index().rename(columns={'code':__symbol_col__})
    return data    
