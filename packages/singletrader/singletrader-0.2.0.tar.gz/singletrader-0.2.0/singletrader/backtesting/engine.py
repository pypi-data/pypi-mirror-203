...
class Engine:
    ...
    def load_data(self,src=None,**kwargs):
        
        src = getattr(self,'src',src) if src is None else src
    ...
    def run_backtest(self,strategy,account,benchmark=None,expanding=False,including_current=False, **kwargs):
        ...
        while True:
            try:
                ...
                if bar.datetime.strftime('%Y-%m-%d') > end_date:
                    break
                elif bar.datetime.strftime('%Y-%m-%d')<start_date:
                    continue
                _strategy.begin_bar()
                ...
                if not including_current:
                    _strategy.am.update_bar(bar)
                _strategy.last_trade_date=bar.datetime
    ...
    def get_all_info(self, key='net_value'):
