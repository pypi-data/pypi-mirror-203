...
from singletrader.datasdk.sql.dataapi import get_price,get_valuation,get_acution

from jqdatasdk import get_all_securities,get_trade_days
...
def get_security_info(**kwargs):
    ...
    return instruments
# get_security_info = jq.get_all_securities
...
APIS = {
    "bar_api":get_price,
    "valuation_api":get_valuation,
    "auction_api":get_acution,
}
...
class DumpDataBase:
    ...
    def _get_source_data(self, code: str) -> pd.DataFrame:
        ...
        if self.api == 'jq_bar_api':
            df = api(universe=[code],start_date=self.start_date,factor_list=factor_list,end_date=self.end_date, trade_date=self.trade_date).reset_index()
        elif self.api != 'jq_api':
            df = api(universe=[code],start_date=self.start_date,factor_list=factor_list,end_date=self.end_date, trade_date=self.trade_date).reset_index()
        else:
            df = api(universe=[code],start_date=self.start_date,end_date=self.end_date).reset_index()
    ...
    def _load_instruments(self):
        ...
        if "type" in instruments.columns:
            instruments = instruments[instruments.type == 'stock']
        if (self.INSTRUMENTS_END_FIELD in instruments.columns) and (self.INSTRUMENTS_START_FIELD in instruments.columns):
            instruments = instruments[(instruments[self.INSTRUMENTS_END_FIELD] > self.start_date_datetime) & \
                                    (instruments[self.INSTRUMENTS_START_FIELD] <= self.end_date_datetime)]
...
def DumpAll():
    DumpDataAll(api="bar_api")()
    DumpDataAll(api="valuation_api")()
    # DumpDataAll(api="auction_api")()

def Update(end_date=last_date_str):
    DumpDataUpdate(api="valuation_api",end_date=end_date)._dump_features()#聚宽备用股票拓展行情2
    # DumpDataUpdate(api="auction_api")._dump_features()
    DumpDataUpdate(api='bar_api',end_date=end_date)() #更新股票行情数据
