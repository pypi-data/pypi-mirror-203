...
def xs_correlation(x, y,method='normal'):
    ...
    xy = xy.dropna()
    if method == 'normal':
        xy_corr = xy.groupby(level=0).apply(lambda df: df.corr().iloc[0, 1])
    elif method == 'rank':
        xy_corr = xy.groupby(level=0).apply(lambda df: df.rank().corr().iloc[0, 1])
    return xy_corr
...
def get_factor_ic(factor_data, ret_data, is_event=False,method='normal'):
    ...
    # fr = pd.concat([factor_data,ret_data],axis=1).dropna()
