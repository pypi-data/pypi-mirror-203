...
class ExplainerWrapper:
    ...
    def __get_explainer_type(self, model_object: Any) -> ExplainerType:
        ...
        if self._is_pdp_or_mace_list(base_classes):
            base_classes_omni = list(map(lambda x: x.__module__ + '.' +
                                         x.__name__, inspect.getmro(type(model_object[0]))))
            if (len(model_object) > 1):
                base_classes_omni_transformer = list(map(lambda x: x.__module__ + '.' +
                                                         x.__name__, inspect.getmro(type(model_object[1]))))
    ...
    def __get_explainer_helper(self, explainer_object) -> BaseExplainer:
        ...
        if explainer_type == ExplainerType.PDP_TABULAR or \
                explainer_type == ExplainerType.MACE_TABULAR:
    ...
    def __is_pdp_tabular(self, base_classes: List[str], obj) -> bool:
        return ('omnixai.explainers.tabular.auto.TabularExplainer' in base_classes)\
             and ('pdp' in obj.explainer_names)

    def __is_mace_tabular(self, base_classes: List[str], obj) -> bool:
        return 'omnixai.explainers.tabular.auto.TabularExplainer' in base_classes\
             and ('mace' in obj.explainer_names)
