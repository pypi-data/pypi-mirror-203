...
def get_cmdclass_from_cfg():
    try:
        with open("builder_temp_config.json") as file:
            data = json.load(file)
    except: data = ["py3", "none", "any"]
    return generate_cmdclass(*data)
...
DEFAULT_ARGS = dict(
	...
	package_data={"pciw": globalizer(os.path.join(os.path.dirname(__file__), "pciw", "data"))},
...
setuptools.setup(**DEFAULT_ARGS, cmdclass=get_cmdclass_from_cfg())