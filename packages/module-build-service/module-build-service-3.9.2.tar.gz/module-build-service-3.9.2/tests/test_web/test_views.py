...
class TestSubmitBuild:
    ...
    def test_submit_build_wrong_stream(self, mocked_scm, mocked_get_user):
        ...
        assert data["message"] == (
            ...
            'does not match the stream determined by the branch "master"'
        )
        assert data["error"] == "Bad Request"

    @pytest.mark.parametrize("dashes_allowed", (True, False))
    @pytest.mark.parametrize("yaml_file", ("testmodule.yaml", "testmodule-dashed-stream.yaml"))
    @patch("module_build_service.web.auth.get_user", return_value=user)
    @patch("module_build_service.common.scm.SCM")
    def test_submit_build_dashes_in_stream(self, mocked_scm, mocked_get_user, yaml_file,
                                           dashes_allowed):
        FakeSCM(
            mocked_scm,
            "testmodule",
        ...
        with patch.object(mbs_config.Config, "allow_dashes_in_svc",
                          new_callable=PropertyMock) as da, \
             patch.object(mbs_config.Config, "allow_stream_override_from_scm",
                          new_callable=PropertyMock) as asofs:
            da.return_value = dashes_allowed
            asofs.return_value = True
            rv = self.client.post(
                "/module-build-service/1/module-builds/",
                data=json.dumps({
                    "branch": "master",
        ...
        if not dashes_allowed and 'dash' in yaml_file:
            assert data["status"] == 400
            assert data["message"] == "Dashes are not allowed in the stream value"
            assert data["error"] == "Bad Request"
        else:
            assert data["name"] == "testmodule"
            if 'dash' in yaml_file:
                assert data["stream"] == "some-stream"
            else:
                assert data["stream"] == "master"
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            {"require_overrides": "platform:f28"},
            {"rpm_component_ref_overrides": {"pkg_name": ["hash"]}},
            {"rpm_component_ref_overrides": {"pkg_name": 28}},
            {"rpm_component_ref_overrides": "pkg_name:hash"},
        ),
    )
    @patch("module_build_service.web.auth.get_user", return_value=user)
    ...
    def test_submit_build_invalid_override(self, mocked_scm, mocked_get_user, optional_params):
        ...
        elif "rpm_component_ref_overrides" in optional_params:
            msg = (
                "The \"rpm_component_ref_overrides\" parameter must be an object with the keys "
                "as component names and the values as strings of refs"
            )
        assert data == {"error": "Bad Request", "message": msg, "status": 400}

    @patch("module_build_service.web.auth.get_user", return_value=user)
    @patch("module_build_service.common.scm.SCM")
    def test_submit_rpm_component_ref_overrides(self, mocked_scm, mocked_get_user):
        optional_params = {"rpm_component_ref_overrides": {"pkg_name": "ref_override"}}
        ...
        xmd_override = mmd.get_xmd()["mbs"]["rpm_component_ref_overrides"]
        assert xmd_override == optional_params["rpm_component_ref_overrides"]
