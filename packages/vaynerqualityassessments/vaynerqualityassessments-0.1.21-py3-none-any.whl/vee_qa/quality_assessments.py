...
class QualityAssessments:
    ...
    def comparison_with_previous_data(self,df,name_of_df,cols_to_check=['impressions','likes'],perc_increase_threshold=20,
        ...
        client_log_dir = os.path.join(base_log_dir, self.util.client_name)
        historic_comparison_folder = os.path.join(client_log_dir, 'Historic df Comparison (Do Not Delete)')

        if os.path.isdir(historic_comparison_folder) == False:
            os.mkdir(historic_comparison_folder)
        if os.path.exists(os.path.join(historic_comparison_folder, f'{name_of_df}_previous_totals.json')) ==False:
            self.util.write_json(new_dict,f'{name_of_df}_previous_totals',file_type='append',folder=historic_comparison_folder)
            qual_logger.logger.info(f"Creation of {name_of_df}_previous_totals")
            qual_logger.logger.info(new_dict)
            return
        else:
            
            old_dict_file = self.util.read_json(f'{name_of_df}_previous_totals',folder=historic_comparison_folder,
                                                    file_type='append')
        ...
        self.util.write_json(new_dict,f'{name_of_df}_previous_totals',file_type='append',
                             folder=historic_comparison_folder)
