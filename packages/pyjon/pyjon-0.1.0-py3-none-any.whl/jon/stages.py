...
from .datatypes import JONData, JONDict, JONKey, JONList, JONNumber, JONQuotedString, JONString
...
class ParsingStageKey(Enum):
    ...
    # Stage that defines a string value or a key in an unknown block stage
    VALUE_STRING_UNKNOWN = "VALUE_STRING_UNKNOWN"
    ...
    # Stage that defines the end of a quoted string
    END_QUOTED_STRING = "END_QUOTED_STRING"
...
class ParsingDecision:
    ...
    def __init__(
        ...
        # In a default decision, the queue is restored from the start of the stage
...
def stage_configuration(key, parent_stage: Optional[ParsingStage]) -> ParsingStageConfiguration:
    ...
    if key == ParsingStageKey.INDENT_LIST:
        # First letter can either be a key or a string value: the stage move forward to the special
        # VALUE_STRING_UNKNOWN stage
        parent_block_stage_status_unknown = (
            parent_stage is None or
            parent_stage.block_stage_status == BlockStageStatus.UNKNOWN
        )
        letter_decision_key = (
            ParsingStageKey.VALUE_STRING_UNKNOWN if parent_block_stage_status_unknown
            else ParsingStageKey.VALUE_STRING
        )
    ...
    if key == ParsingStageKey.VALUE_STRING:
        inline_decisions = [
            ParsingDecision(default=True, status=ParsingStatus.QUIT),
    ...
    if key == ParsingStageKey.VALUE_STRING_UNKNOWN:
        inline_decisions = [
            ParsingDecision(
                default=True,
    ...
    if key == ParsingStageKey.VALUE_QUOTED_STRING:
        return ParsingStageConfiguration(
            ...
            decisions=[
                ParsingDecision(
                    regex=ParsingRegexKey.QUOTE,
                    key=ParsingStageKey.END_QUOTED_STRING,
    ...
    if key == ParsingStageKey.END_QUOTED_STRING:
        inline_decisions = [
            ParsingDecision(default=True, status=ParsingStatus.QUIT),
        ] if parent_stage.inline else [
            ParsingDecision(regex=ParsingRegexKey.NEWLINE, status=ParsingStatus.QUIT),
    ...
    if key == ParsingStageKey.COMMENT:
        return ParsingStageConfiguration(
            regex=ParsingRegexKey.NO_SPACE,
            decisions=[
                ParsingDecision(regex=ParsingRegexKey.NEWLINE, status=ParsingStatus.QUIT),
                ParsingDecision(
                    regex=ParsingRegexKey.NO_SPACE,
...
class ParsingStage:
    ...
    def __init__(
        ...
        if data is not None:
            ...
            self.parent.data.attach(self.data)
        else:
            self.data = None
    ...
    def _parse(self) -> Tuple[ParsingStatus, Optional[ParsingStage]]:
        ...
        if self._config.has_indent:
            ...
            whitespaces_count += 4 * whitespaces['\t']
        ...
        for decision in self._config.decisions:
            if PARSING_REGEXES[decision.regex].match(char) or decision.default:
                ...
                if not decision.skip_whitespaces_count:
                    if char == "\n":
                        if whitespaces_count != 0:
                            self._validation_errors.append(ParsingValidationError(
                                "W002",
