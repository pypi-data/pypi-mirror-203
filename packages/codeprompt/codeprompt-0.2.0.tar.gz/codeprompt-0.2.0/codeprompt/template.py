...
class TemplateManager:
    ...
    def build_message_from_template(self, template_name, substitutions={}):
        ...
        template_substitutions, overrides = self.extract_template_run_overrides(
            source.metadata
        )
    ...
    def load_templates(self):
        self.log.debug(
            "Loading templates from dirs: %s" % ", ".join(self.template_dirs)
        )
    ...
    def get_template_and_variables(self, template_name):
        try:
            ...
            return None, None
        template_source = self.templates_env.loader.get_source(
            self.templates_env, template_name
        )
