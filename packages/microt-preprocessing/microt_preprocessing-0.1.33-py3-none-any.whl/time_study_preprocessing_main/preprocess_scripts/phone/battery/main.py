            ...
            if not path.exists(day_file_save_path):
                ...
                if hours_with_target_file != 0:
                    ...
                    if not os.path.exists(day_file_name_battery_minute):
                        ...
                        df_battery_minute["PARTICIPANT_ID"] = [pid] * len(df_battery_minute)
                        df_battery_minute["NUMERICAL_ID"] = [numerical_id] * len(df_battery_minute)
                        ...
                        df_battery_hour["PARTICIPANT_ID"] = [pid] * len(df_battery_hour)
                        df_battery_hour["NUMERICAL_ID"] = [numerical_id] * len(df_battery_hour)
                        df_battery_hour.to_csv(day_file_name_battery_hour, index=False)

                        df_battery_day = get_battery_matrix_day(df_battery_hour)
                        df_battery_day["PARTICIPANT_ID"] = [pid] * len(df_battery_day)
                        df_battery_day["NUMERICAL_ID"] = [numerical_id] * len(df_battery_day)
