from brci.apis import perf as PerfApi
from brci.handles.build import (
    get_extra_env,
    get_jenkins_info,
    get_build_result,
    write_result,
)
from brci.schemas import jenkins as JenkinsSchema
from brci.schemas import test_case as TestCaseSchema

from brci.utils.csv_utils import load_csv, write_csv, load_perf_csv
from brci.utils.json_utils import load_json
from brci.utils.file_utils import load_kvs
from brci.utils.pd_utils import PD
...
def add_perf(csv_path=None):
    ...
    buildResult.result = perf_csv_format(get_build_result(csv_path)["csv"])
...
def get_compare_conf(conf_path):
    ...
    out = {
        ...
        "reportCol": os.getenv(
            "reportCol", "preValue,preCompare,currValue,avgCompare,avgValue"
        ).split(","),
        "value": {
            "preValue": [],
            "currValue": [],
