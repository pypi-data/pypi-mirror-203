...
class Service:
    ...
    async def __call__(self, *args, **kwargs):
        service_type = self.config.get("service_type", "asgi")

        if service_type == "asgi":
            return await self.asgi_app(*args, **kwargs)
        elif service_type == "wsgi":
            return self.wsgi_app(*args, **kwargs)
        else:
            raise TypeError(f"Unknown Service Type {service_type}")
    ...
    def __init__(self, root, config: Dict = None):
        ...
        self.asgi_app: ASGIFramework = AsyncioWSGIMiddleware(
            self.wsgi_app,
            max_body_size=2 ** 32
        )
