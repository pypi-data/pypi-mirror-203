...
def _extract_df_columns(
    ...
    if version in (
        ...
        ts_col = pl.col(str(OBSERVED_AT_FEATURE)).fill_null(pl.col(str(CHALK_TS_FEATURE))).dt.convert_time_zone("UTC")
        if output_ts:
    ...
    for col, dtype in zip(df.columns, df.dtypes):
        if version in (
            ...
            if isinstance(dtype, pl.Datetime):
                ...
                decoded_stmts.append(pl.col(col).dt.convert_time_zone("UTC"))
            else:
...
class DatasetImpl(Dataset):
    ...
    def recompute(self, features: Optional[List[Union[Feature, Any]]] = None):
        if len(self.revisions) == 0:
            raise IndexError("No revisions exist for dataset")
        if self._client._config.branch is None:
            raise ValueError("No branch was provided when constructing the Chalk Client")
        ...
        new_revision = self._client._recompute_dataset(
            ...
            branch=self._client._config.branch,
