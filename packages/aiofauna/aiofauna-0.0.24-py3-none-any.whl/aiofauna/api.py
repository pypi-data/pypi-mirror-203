...
class RequestBodyJSONDecoder(JSONDecoder):
    def __init__(self):
        super().__init__(object_hook=self.object_hook)
        
    def object_hook(self, obj):
        if isinstance(obj, dict):
            return {k: self.object_hook(v) for k, v in obj.items()}
        elif isinstance(obj, list):
            return [self.object_hook(v) for v in obj]
        elif isinstance(obj, str):
            if obj.startswith('datetime:'):
                return datetime.strptime(obj[9:], '%Y-%m-%d %H:%M:%S.%f')
            elif obj.startswith('date:'):
                return datetime.strptime(obj[5:], '%Y-%m-%d').date()
            elif obj.startswith('time:'):
                return datetime.strptime(obj[5:], '%H:%M:%S.%f').time()
            elif obj.startswith('uuid:'):
                return UUID(obj[5:])
            elif obj.startswith('bytes:'):
                return base64.b64decode(obj[6:])
            else:
                return obj
        elif isinstance(obj, (int, float, bool, type(None))):
            return obj
        elif isinstance(obj, (datetime, date, time, UUID)):
            return obj
...
custom_decoder = RequestBodyJSONDecoder()
...
async def extract_parameters(request: Request, params: dict):
    ...
    for name, param in params.items():
        ...
        if type_ in (str, int, float, bool) and name in request.match_info:
            ...
            data = await request.json(loads=custom_decoder.decode)
