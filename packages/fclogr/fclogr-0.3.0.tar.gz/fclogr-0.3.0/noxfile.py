...
def formatters(session: nox.Session):
    ...
    session.run("black", *posargs, *LINT_FILES, "tests")
...
def _add_frag(
    ...
    session.log(f"Git tag message written to {git_msg_file}")
    return frag_heading, raw_frag_lines, git_msg_file


@nox.session
def add_frag(session: nox.Session):
    session.install("tomcli[tomli]")
    version = session.run(
        "tomcli-get",
        "--reader=tomllib",
        "pyproject.toml",
    ...
    _add_frag(session, "FRAG.md", "NEWS.md", version)
...
def bump(session: nox.Session):
    ...
    install(session, "flit", "tomcli[tomlkit]")
    ...
    whl = tuple(Path("dist").glob("*.whl"))
    assert len(whl) == 1
    whl_str = str(whl[0])
    install(session, whl_str)
    ...
    git_msg_file = _add_frag(session, "FRAG.md", "NEWS.md", version)[2]
...
def _sign_artifacts(session: nox.Session) -> None:
    ...
    artifacts = _get_artifacts(["tar.gz", "whl"])
...
def publish(session: nox.Session):
    ...
    artifacts = tuple(_get_artifacts(extra_allowed=False, expected_count=4))
