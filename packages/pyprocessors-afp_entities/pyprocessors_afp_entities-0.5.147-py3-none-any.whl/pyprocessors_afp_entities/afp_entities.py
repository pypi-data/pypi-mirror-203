...
def consolidate_candidate(
    ...
    for ann in sorted_annotations:
        if params.resolve_lastnames:
            lastnames = person_lastnames(ann)
    ...
    for k, g in groupby(sorted_annotations, keyfunc):
        ...
        if k in ['person', 'location', 'organization']:
            ...
            for r in groups[k].ranges():
                ...
                dtermids = [wt.term.identifier for wt in wterms if wt.status == 'D']
                terms = [wt.term for wt in wterms if wt.term.identifier not in dtermids]
                if len(terms) > 0:
                    a.terms = terms
                else:
...
def person_lastnames(a: Annotation):
