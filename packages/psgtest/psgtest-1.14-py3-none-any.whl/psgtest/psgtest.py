...
version = '1.14 Released 14-Apr-2023'
...
def make_window(sp_to_mline_dict=None, sp_to_filename=None):
    ...
    regression_frame_layout = [[sg.CBox('Kill test after', default=False, k='-REGRESSION TEST-'), sg.Input(15, s=4, justification='r', k='-REGRESSION SECONDS-'), sg.Text('seconds (regression tests)')],
...
def main():
            ...
            if pipe_output:
                ...
                if values['-REGRESSION TEST-']:
                    try:
                        ...
                        seconds = 15
                    window.start_thread(lambda: timer_thread(window, sp, seconds),'-TIMER THREAD DONE-')
    ...
    while True:
        ...
        if isinstance(event, tuple):
            ...
            for test_num in range(int(values['-STRESS RUNS-'])):
                sg.cprint(f'Starting Iteration: {test_num}', c='white on blue')
