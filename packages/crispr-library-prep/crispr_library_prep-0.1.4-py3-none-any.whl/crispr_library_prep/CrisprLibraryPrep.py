...
@typechecked
def get_target_coverage_per_sample(gDNA_amount: float, moi: float, sample_name:str, perfection_rate: float, guide_library_size:int, genome_conc_ng: float = _GENOME_CONC_NG, target_coverage_input=None, target_coverage_percentage: float=0.70, max_target_coverage_input: int = 2000, read_intervals_count: int= 20):
    ...
    if target_coverage_input is not None:
        ...
        if target_coverage_input > PCR1_input_coverage:
            ...
            target_read_amount_input = int(target_read_amount_input)


    # Visualize
    print(f"gDNA: {gDNA_amount}; MOI: {int(moi)}; Desired Coverage={int(target_coverage_input)}, Suggested Coverage={int(target_coverage_suggested)}, Max Coverage={int(PCR1_input_coverage)}, \n Reads for Desired Coverage={target_read_amount_input}, Reads for Suggested Coverage={int(target_read_amount_suggested)}")
    ...
    plt.axvline(x=target_read_amount_input, color="black", linestyle="dashed", label=f"For desired coverage {int(target_coverage_input)}, target reads for coverage={target_read_amount_input}")
