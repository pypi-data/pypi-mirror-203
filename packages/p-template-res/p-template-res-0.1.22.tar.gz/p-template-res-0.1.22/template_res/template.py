...
def updateResources(rootDir):
    ...
    getOssResource(rootDir, "http://mecord-m.2tianxin.com/res/music_20230419.zip", "1dfd16739a11ee9283fae0ef53be570d", "music.zip.py")
...
ddd = "E:\\template\\template_python\\auto_res_oss\\res\\music"
idx = 0
for root,dirs,files in os.walk(ddd):
    for file in files:
        ...
        name = file[0:file.index(".")].lower()
        ext = file[file.index("."):].lower()
        os.rename(os.path.join(root, file), os.path.join(root, f"music_{idx}{ext}"))
        idx+=1
    if root != files:
        break