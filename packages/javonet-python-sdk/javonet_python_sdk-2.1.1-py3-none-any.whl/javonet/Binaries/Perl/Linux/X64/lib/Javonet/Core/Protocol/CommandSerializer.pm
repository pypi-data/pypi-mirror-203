...
sub serialize_primitive {
    ...
    if(isint($payload_item)){
        my @serialized_int = TypeSerializer->serializeInt($payload_item);
        return @serialized_int;
    }
    if(type($payload_item) eq "FLOAT"){
        my @serialized_double = TypeSerializer->serializeDouble($payload_item);
        return @serialized_double;
    }
    else{
        my @serialized_string = TypeSerializer->serializeString($payload_item);
