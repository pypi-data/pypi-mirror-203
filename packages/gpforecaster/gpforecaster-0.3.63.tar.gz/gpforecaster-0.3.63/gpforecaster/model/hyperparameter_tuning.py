...
def single_trial(
    ...
    yield (val_loss, hyperparameters)
...
def optimize_hyperparameters_bayesian(
    ...
    def wrapped_single_trial(**hyperparameters):
        generator_result = single_trial(
            dataset_name, hierarchical_data, hyperparameters, gp_type
        )
        val_loss, _ = next(generator_result)
        return val_loss
    ...
    log_and_print_best_hyperparameters(
        gp_type, dataset_name, best_hyperparameters, results.fun
    )
