...
class GPF:
    ...
    def hierarchical_data_summary(self, bottom_series=None):
        ...
        summary = {
            "total": np.sum(bottom_series, axis=1),
            "groups": {},
            "group_ele": {},
        ...
        for group in groups_names:
            ...
            data_g = np.zeros((n_samples, n_elements_group))

            for group_idx, element_name in enumerate(group_elements):
                group_element_active = np.where(groups_idx == group_idx, 1, 0).reshape(
                    (1, -1)
                )
                data_g[:, group_idx] = np.sum(
                    group_element_active * bottom_series, axis=1
                )

                if element_name not in summary["group_ele"]:
                    summary["group_ele"][element_name] = np.sum(
                        group_element_active * bottom_series, axis=1
                    )

            summary["groups"][group] = np.sum(data_g, axis=1)
    ...
    def concatenate_arrays(nested_dict):
        arrays = list()
        arrays.append(nested_dict["total"])

        for group_array in nested_dict["groups"].values():
            arrays.append(group_array)
        for group_ele_array in nested_dict["group_ele"].values():
            arrays.append(group_ele_array)

        arrays.extend([series for series in nested_dict["bottom"].T])
        concatenated_array = np.column_stack(arrays)
    ...
    def _train_loop_iteration(
        ...
        if early_stopping and epoch % 5 == 0:
            ...
            val_loss = self.validate(
                model, val_x, val_y, train_y, likelihood
            )
    ...
    def _log_training_information(
        ...
        process = psutil.Process(os.getpid())
        mem = process.memory_info().rss / (1024 ** 3)
        self.logger_train.info(f"Training used {mem:.3f} GB of RAM")
