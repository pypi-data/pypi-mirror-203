...
class GPF:
    ...
    def _train_loop_iteration(
        ...
        if early_stopping and epoch % 5 == 0:
            model.eval()
            likelihood.eval()
            val_loss = self.validate(model, val_x, val_y)
    ...
    def train(
        ...
        """
        train_x, val_x = self.split_train_val(self.train_x, self.groups["h"] * 2)
        train_y, val_y = self.split_train_val(self.train_y, self.groups["h"] * 2)

        likelihood_list, model_list = self._build_model(
            x=train_x, y=train_y, random_init=random_init
        )
        ...
        for epoch in range(epochs):
            for i, single_model in enumerate(model.models):
                single_model.set_train_data(
                    inputs=train_x, targets=train_y[:, i], strict=False
                )
    ...
    def validate(self, model: IndependentModelList, val_x, val_y) -> float:
        """
        Validate the model.
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            for i, single_model in enumerate(model.models):
                single_model.set_train_data(
                    inputs=val_x, targets=val_y[:, i], strict=False
                )

            val_output = model(*tuple(val_x for i in range(self.groups["train"]["s"])))
            val_loss = -self.mll(val_output, list(torch.transpose(val_y, 0, 1)))
