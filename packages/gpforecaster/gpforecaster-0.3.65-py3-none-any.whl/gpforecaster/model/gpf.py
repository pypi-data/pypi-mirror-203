...
class GPF:
    ...
    def __init__(
        ...
        self.train_x = torch.from_numpy(np.asarray(groups["train"]["x_values"])).to(
            self.device
        )
        ...
        self.test_x = self.test_x.type(torch.Tensor).to(self.device)
    ...
    def hierarchical_data_summary(self, bottom_series=None):
        ...
        for group in groups_names:
            ...
            data_g = torch.zeros((n_samples, n_elements_group))
            ...
            summary["groups"][group] = torch.sum(data_g, dim=1)

        return summary

    @staticmethod
    def concatenate_arrays(nested_dict):

        total = nested_dict["total"]
        group_arrays = [group_array for group_array in nested_dict["groups"].values()]
        group_ele_arrays = [
            group_ele_array for group_ele_array in nested_dict["group_ele"].values()
        ]
        bottom_arrays = [series for series in nested_dict["bottom"].T]
    ...
    @staticmethod
    def split_train_val(data, num_val):
        val_size = num_val
