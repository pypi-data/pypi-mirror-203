...
class GPF:
    ...
    def __init__(
        ...
        self.train_x = torch.from_numpy(np.asarray(groups["train"]["x_values"])).to(torch.float32).to(
            self.device
        )
    ...
    def _preprocess(self, groups, device):
        ...
        groups["train"]["data"] = torch.tensor(
            groups["train"]["data"], device=device, dtype=torch.float32
        )
        groups["predict"]["data_matrix"] = torch.tensor(
            groups["predict"]["data_matrix"], device=device, dtype=torch.float32
        )
    ...
    def hierarchical_data_summary(self, bottom_series=None):
        ...
        for group in groups_names:
            ...
            data_g = torch.zeros((n_samples, n_elements_group)).to(torch.float32).to(self.device)
            ...
            for group_idx, element_name in enumerate(group_elements):
                ...
                data_g[:, group_idx] = torch.sum(
                    group_element_active * summary["bottom"], dim=1
                ).to(torch.float32).to(self.device)

                if element_name not in summary["group_ele"]:
                    summary["group_ele"][element_name] = torch.sum(
                        group_element_active * summary["bottom"], dim=1
                    ).to(torch.float32).to(self.device)

            summary["groups"][group] = torch.sum(data_g, dim=1).to(torch.float32).to(self.device)

        return summary

    def concatenate_arrays(self, nested_dict):
        total = nested_dict["total"]
        group_arrays = [group_array.to(self.device) for group_array in nested_dict["groups"].values()]
        group_ele_arrays = [group_ele_array.to(self.device) for group_ele_array in nested_dict["group_ele"].values()]
    ...
    def _build_model(
        ...
        changepoints = torch.tensor(changepoints, dtype=torch.float32, device=self.device)
    ...
    def validate(
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            val_pred = torch.zeros((val_x.shape[0], self.s)).to(torch.float32).to(self.device)
    ...
    def predict(
        ...
        for ts in range(self.s):
            ...
            pred_std_scaled[:, ts] = np.sqrt(predictions[ts].variance.cpu().detach().numpy())
