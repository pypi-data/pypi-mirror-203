...
"""outlineApp.py - a simple GUI for the compilation of outline-files"""
...
class outlineApp(tkinter.Tk):
    def __init__(self):
        super().__init__()
        self.withdraw()
        self.title('outline App')
        ...
        if 'html' in outlineParser.targets or 'HTML' in outlineParser.targets:
            self.target = 'html'
        elif len(outlineParser.targets) == 1:
            self.target = list(outlineParser.targets)[0]
        else:
            self.target = ''
        ...
        if not self.worker or self.on_cancel():
            ...
            if self.names:
                ...
                self.worker = threading.Thread(
                    target = outlineParser.batch_process,
...
if __name__ == '__main__':
    ...
    if not outlineParser.main(called_from_app=True):
        app = outlineApp()
        app.mainloop()
