...
"""outlineServer.py - starts a server (if not already running) for the
...
class outlineLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "outline-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = outlineCPUBoundTasks(self.lsp_data)
        self.blocking = outlineBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from outlineParser import batch_process
        args = argstr.split(' ')
    ...
    #     from outlineParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new outlineServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if grammar_src and not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('outline_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    outline_lsp = outlineLanguageServerProtocol()
    lsp_table = outline_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', outline_lsp.simply_compile)
    outline_server = Server(rpc_functions=lsp_table,
                        cpu_bound=outline_lsp.cpu_bound.lsp_table.keys(),
                        blocking=outline_lsp.blocking.lsp_table.keys(),
                        connection_callback=outline_lsp.connect,
                        server_name="outlineServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        outline_server.echo_log = True if port >= 0 and host else False
        msg = outline_server.start_logging(log_path.strip('" \''))
        if outline_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        outline_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            outline_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing outline-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python outlineServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python outlineServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python outlineServer.py --stream\n'
             + '    python outlineServer.py --stopserver\n'
             + '    python outlineServer.py --status\n'
             + '    python outlineServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python outlineServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
