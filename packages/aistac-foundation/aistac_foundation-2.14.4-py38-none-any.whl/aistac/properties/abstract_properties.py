...
class AbstractPropertyManager(ABC):
    ...
    def get_connector_contract(self, connector_name: str) -> ConnectorContract:
        ...
        _schema, _, _ = ConnectorContract.parse_address_elements(uri=self.get(self.join(_connector_key, 'raw_uri')))
        _module_name = self.get(self.join(_connector_key, 'raw_module_name'))
        _handler = self.get(self.join(_connector_key, 'raw_handler'))
        if not str(_module_name).startswith('${') and not str(_handler).startswith('${'):
            for _package in ['ds_discovery', 'ds_connectors']:
                _schema_module_name = f'{_package}.handlers.{_schema.lower()}_handlers'
                _schema_handler = f'{_schema.title()}PersistHandler'
                if HandlerFactory.check_handler(_schema_module_name, _schema_handler):
                    self.set(self.join(_connector_key, 'raw_module_name'), _schema_module_name)
                    self.set(self.join(_connector_key, 'raw_handler'), _schema_handler)
