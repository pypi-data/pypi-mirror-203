...
class AbstractComponent(ABC):
    ...
    def load_canonical(self, connector_name: str, reset_changed: bool=None, has_changed: bool=None,
        ...
        """
        if self.pm.has_connector(connector_name):
            connector_contract = self.pm.get_connector_contract(connector_name)
            _module_name, _handler = self._from_handler(schema=connector_contract.schema)
            if _module_name != connector_contract.handler or _handler != connector_contract.handler:
                self.remove_connector_contract(connector_name)
                self.add_connector_uri(connector_name, uri=connector_contract.raw_uri, **connector_contract.raw_kwargs)
