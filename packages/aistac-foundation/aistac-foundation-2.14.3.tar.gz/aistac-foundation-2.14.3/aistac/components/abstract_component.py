...
class AbstractComponent(ABC):
    ...
    def load_canonical(self, connector_name: str, reset_changed: bool=None, has_changed: bool=None,
        ...
        """
        if self.pm.has_connector(connector_name):
            current_cc = self.pm.get_connector_contract(connector_name)
            _module_name, _handler = self._from_handler(schema=current_cc.schema)
            if _module_name != current_cc.handler or _handler != current_cc.handler:
                self.remove_connector_contract(connector_name)
                new_cc = ConnectorContract(uri=current_cc.raw_uri, module_name=_module_name, handler=_handler,
                                           version=current_cc.raw_version, **current_cc.raw_kwargs)
                self.add_connector_contract(connector_name, connector_contract=new_cc)
