...
class AbstractComponent(ABC):
    ...
    def load_canonical(self, connector_name: str, reset_changed: bool=None, has_changed: bool=None,
        ...
        """
        if self.pm.has_connector(connector_name):
            old_cc = self.pm.get_connector_contract(connector_name)
            _module_name, _handler = self._from_handler(schema=old_cc.schema)
            if _module_name != old_cc.handler or _handler != old_cc.handler:
                self.remove_connector_contract(connector_name)
                new_cc = ConnectorContract(uri=old_cc.raw_uri, module_name=old_cc.raw_module_name,
                                           handler=old_cc.raw_handler, version=old_cc.raw_version, **old_cc.raw_kwargs)
