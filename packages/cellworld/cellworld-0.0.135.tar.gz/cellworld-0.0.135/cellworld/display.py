...
class Display:
    ...
    def plot_clusters(self, clusters, colors: list=list(mcolors.TABLEAU_COLORS.keys()), show_centroids: bool=True):
        ...
        for cn, cluster in enumerate(clusters):
            ...
            color = adjust_color_brightness(mcolors.TABLEAU_COLORS[colors[cn % len(colors)]], .8)
            for i, sl in enumerate(cluster):
                self.ax.plot([s.x for s in sl], [s.y for s in sl], c=color, linewidth=2, alpha=.5) #alpha_values[i])

        if show_centroids:
            print("Clusters")
            for cn, cluster in enumerate(clusters):
                color = adjust_color_brightness(mcolors.TABLEAU_COLORS[colors[cn % len(colors)]], 1.2)
