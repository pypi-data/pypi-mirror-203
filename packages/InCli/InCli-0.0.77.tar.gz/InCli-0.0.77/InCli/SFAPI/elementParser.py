...
def create_LU_limits():
  LU = {
    ...
    'Email Invocations': 0,
    'future calls': 0,
    'queueable jobs added to the queue': 0,
    'Mobile Apex push calls': 0
  }
  return LU
...
def parseUserDebug(context):
    ...
    if is_in_operation(context,'USER_DEBUG'):
        ...
        if  obj['output'].startswith('*** getCpuTime() ***'):
            ...
            append_LU_default(context,'CPU time',chs[4])
        ...
        if  obj['output'].startswith('*** getQueries() ***'):  
            ...
            append_LU_default(context,'SOQL queries',chs[4])
