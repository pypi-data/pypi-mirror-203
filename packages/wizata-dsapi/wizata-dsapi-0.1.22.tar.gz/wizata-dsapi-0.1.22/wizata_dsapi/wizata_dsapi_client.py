...
class WizataDSAPIClient:
    ...
    def get(self, obj=None, script_name=None, experiment_key=None, model_key=None, template_key=None):
        """
    ...
    def update(self, obj):
        ...
        if isinstance(obj, Script):
            ...
            response = requests.put(self.__url() + "templates/",
                                     headers=self.__header(),
                                     data=json.dumps(obj.to_json()))
            if response.status_code == 200:
                return
    ...
    def upsert_template(self, key: str, name: str):
        ...
        """
        try:
            to_update = self.get(template_key=key)
