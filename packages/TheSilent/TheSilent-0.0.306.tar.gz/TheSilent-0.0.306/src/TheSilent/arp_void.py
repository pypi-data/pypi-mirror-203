...
def arp_void(router):
    ...
    print(CYAN + "mac address: " + mac + " | ip address: " +
          router + " | interface: " + str(interface[1][1]))
    ...
    while True:
        ...
        try:
            ...
            if platform == "linux":
                super_socket = socket.socket(
                    socket.AF_PACKET, socket.SOCK_RAW, socket.ntohs(3))

            else:
                print(
                    RED + "Unsupported platform! Linux is required for this tool!")
                sys.exit()
