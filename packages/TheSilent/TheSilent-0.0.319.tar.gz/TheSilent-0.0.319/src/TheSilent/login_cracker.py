...
def login_cracker(url, user_name="", word_list=" ", secure=True, tor=False):
    ...
    for activity in form_activities:
        try:
            ...
            if action[0] != "" and not my_url in action[0]:
                if my_url.endswith("/") and action[0] != my_url + "/" or my_url.endswith("/") and action[0] != my_url:
                    my_link = my_url + action[0]

                elif action[0] == my_url + "/" or action[0] == my_url:
                    my_link = my_url
