            ...
            try:
                ...
                if tor:
                    result = web_session.get(
                        j,
                        verify=False,
                        headers={"User-Agent": return_user_agent()}, cookies=mal_cookie, proxies=tor_proxy, timeout=(5, 30)).text

                else:
                    result = web_session.get(
                        j,
                        verify=False,
                        headers={"User-Agent": return_user_agent()}, cookies=mal_cookie, timeout=(5, 30)).text
                    ...
                    for ii in mal_sql:
                        try:
                            ...
                            if name[0] in activity:
                                ...
                                if action[0] != "" and j not in action[0]:
                                    if j.endswith(
