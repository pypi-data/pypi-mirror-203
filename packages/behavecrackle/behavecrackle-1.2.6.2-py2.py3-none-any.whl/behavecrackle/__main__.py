...
import sys
import six
from behavecrackle.version import VERSION as BEHAVE_VERSION
from behavecrackle.configuration import Configuration
from behavecrackle.exception import ConstraintError, ConfigError, \
    FileNotFoundError, InvalidFileLocationError, InvalidFilenameError, \
    ModuleNotFoundError, ClassNotFoundError, InvalidClassError
from behavecrackle.parser import ParserError
from behavecrackle.runner import Runner
from behavecrackle.runner_util import print_undefined_step_snippets, reset_runtime
from behavecrackle.textutil import compute_words_maxsize, text as _text
from behavecrackle.runner_plugin import RunnerPlugin
# PREPARED: from behavecrackle.importer import make_scoped_class_name
...
You may even leave off the "@" - behavecrackle doesn't mind.
...
def run_behave(config, runner_class=None):
    """Run behavecrackle with configuration (and optional runner class).

    :param config:          Configuration object for behavecrackle.
    ...
    if config.version:
        print("behavecrackle " + BEHAVE_VERSION)
        return 0
    ...
    if not config.format:
        ...
        # -- CASE: Formatter are specified in behavecrackle configuration file.
...
def print_language_list(file=None):
    ...
    * ...
    """
    from behavecrackle.i18n import languages
...
def print_formatters(file=None):
    ...
    """
    from behavecrackle.formatter._registry  import format_items
...
# ---------------------------------------------------------------------------
def main(args=None):
    """Main function to run behavecrackle (as program).
