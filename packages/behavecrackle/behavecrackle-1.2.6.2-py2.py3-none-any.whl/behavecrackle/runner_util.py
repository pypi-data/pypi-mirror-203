...
from behavecrackle import parser
from behavecrackle.exception import \
    FileNotFoundError, InvalidFileLocationError, InvalidFilenameError
from behavecrackle.model_core import FileLocation
from behavecrackle.textutil import ensure_stream_with_encoder
# LAZY: from behavecrackle.step_registry import setup_step_decorators
...
class FeatureScenarioLocationCollector(object):
    ...
    USE CASE:
        behavecrackle feature/foo.feature:10
        behavecrackle @selected_features.txt
        behavecrackle @rerun_failed_scenarios.txt
...
def collect_feature_locations(paths, strict=True):
    ...
    for path in paths:
        if os.path.isdir(path):
            ...
            # -- USE: behavecrackle @list_of_features.txt
...
def load_step_modules(step_paths):
    ...
    from behavecrackle import matchers
    from behavecrackle.step_registry import setup_step_decorators
    step_globals = {
...
def print_undefined_step_snippets(undefined_steps, stream=None, colored=True):
    ...
    if colored:
        ...
        from behavecrackle.formatter.ansi_escapes import escapes
