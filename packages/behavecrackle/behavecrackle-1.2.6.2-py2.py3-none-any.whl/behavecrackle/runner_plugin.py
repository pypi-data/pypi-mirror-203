# -*- coding: UTF-8 -*-
"""
Create a runner as behavecrackle plugin by using its name:

* scoped-class-name, like: "behavecrackle.runner:Runner" (dotted.module:ClassName)
* runner-alias (alias mapping provided in config-file "behavecrackle.runners" section)

.. code-block:: ini

    # -- FILE: behavecrackle.ini
    # RUNNER-ALIAS EXAMPLE:
    # USE: behavecrackle --runner=default features/
    [behavecrackle.runners]
    default = behavecrackle.runner:Runner
"""

from __future__ import absolute_import, print_function
import inspect
from behavecrackle.api.runner import ITestRunner
from behavecrackle.exception import ConfigError, ClassNotFoundError, InvalidClassError, ModuleNotFoundError
from behavecrackle.importer import load_module, make_scoped_class_name, parse_scoped_name
...
class RunnerPlugin(object):
    ...
    * create a runner by using a runner-class
    ...
    .. code-block:: python
        ...
        plugin = RunnerPlugin("behavecrackle.runner:Runner")
        runner = plugin.make_runner(config)

        # -- CASE 1B: Make a runner by using its runner-alias
        # CONFIG-FILE SECTION: "behavecrackle.ini"
        #   [behavecrackle.runners]
        #   one = behavecrackle.runner:Runner
        plugin = RunnerPlugin("one")
        runner = plugin.make_runner(config)

        # -- CASE 2: Make a runner by using a runner-class
        from behavecrackle.runner import Runner as DefaultRunner
    ...
    def validate_class(runner_class, runner_class_name=None):
        ...
        if not inspect.isclass(runner_class):
            ...
            message = "is not a subclass-of 'behavecrackle.api.runner:ITestRunner'"
    ...
    def make_runner(self, config, **runner_kwargs):
        ...
        * using its name (alias-name or scoped-class-name).
        ...
        :raises InvalidClassError: If class is invalid (wrong subclass or not a class).
        :raises ImportError: If module was not found (or other Import-Errors above).
        :raises ConfigError: If runner-alias is not in config-file (section: behavecrackle.runners).
        """
        ...
        if not runner_class:
            ...
            if scoped_class_name == runner_name and ":" not in scoped_class_name:
                # -- CASE: runner-alias is not in config-file section="behavecrackle.runner".
