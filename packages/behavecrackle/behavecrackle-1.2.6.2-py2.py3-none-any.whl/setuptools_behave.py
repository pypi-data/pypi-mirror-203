...
# -*- coding: utf-8 -*-
"""
Setuptools command for behavecrackle.
...
class behave_test(Command):
    """
    Simple test runner that runs 'behavecrackle' via a "setup.py" file.
    This ensures that all requirements are provided before the tests are run.
    """
    command_name = "behave_test"
    description = "Run feature tests with behavecrackle"
    ...
    def behave(self, path):
        behave = os.path.join("bin", "behavecrackle")
        if not os.path.exists(behave):
            # -- ALTERNATIVE: USE: behavecrackle script: behavecrackle = "behavecrackle"
            # -- USE: behavecrackle module (main)
            behave = "-m behavecrackle"
        cmd_options = ""
        if self.tags:
