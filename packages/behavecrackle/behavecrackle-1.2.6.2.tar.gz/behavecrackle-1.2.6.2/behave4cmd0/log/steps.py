...
.. code-block: gherkin
    ...
    Then the command output should not contain the following log records:
        | category | level   | message |
        | bar      | CURRENT | xxx     |
    Then the file "behavecrackle.log" should contain the log records:
        | category | level   | message |
        | bar      | CURRENT | xxx     |
    Then the file "behavecrackle.log" should not contain the log records:
        | category | level   | message |
        | bar      | CURRENT | xxx     |
...
from behavecrackle import given, when, then, step
from behave4cmd0.command_steps import \
    step_file_should_contain_multiline_text, \
    step_file_should_not_contain_multiline_text
from behavecrackle.configuration import LogLevel
from behavecrackle.log_capture import LoggingCapture
import logging
...
class LogRecordTable(object):
    ...
    def annotate_with_row_schema(table, row_schema):
        ...
        :param table:   Table w/ log-records (as :class:`behavecrackle.model.Table`)
