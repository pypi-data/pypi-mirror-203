...
.. code-block:: Gherkin
    ...
    Given I use the behavecrackle model builder with:
        | statement  | name   | tags      | Comment |
        | Scenario   | A1     | @foo      |          |
        | Scenario   | A3     | @foo @bar |          |
        | Scenario   | B3     |           | Untagged |
    When I run the behavecrackle with tags
...
.. code-block:: Gherkin
    ...
    Given I use the behavecrackle model builder with:
        ...
        | Scenario   | B2     | @foo      |          |
        | Scenario   | B3     |           | Untagged |
    When I run the behavecrackle with options "--tags=@foo"
...
# STEP DEFINITIONS:
# -----------------------------------------------------------------------------
@given('a behavecrackle model with')
def step_given_a_behave_model_with_table(context):
    """
    Build a behavecrackle feature model from a tabular description.
...
@when('I run the behavecrackle model with "{hint}"')
