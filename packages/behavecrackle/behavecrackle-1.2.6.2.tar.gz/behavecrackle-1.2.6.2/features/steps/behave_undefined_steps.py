# -*- coding: UTF-8 -*-
"""
Provides step definitions for behavecrackle based on behave4cmd.

REQUIRES:
  * behave4cmd.steplib.output steps (command output from behavecrackle).
"""

from __future__ import absolute_import
from behavecrackle import then
from behavecrackle.runner_util import make_undefined_step_snippet
...
def step_undefined_step_snippets_section_exists(context):
    """
    Checks if an undefined-step snippet section is in behavecrackle command output.
    """
...
def step_undefined_step_snippet_should_exist_for(context, step):
    ...
    in behavecrackle command output (last command).
