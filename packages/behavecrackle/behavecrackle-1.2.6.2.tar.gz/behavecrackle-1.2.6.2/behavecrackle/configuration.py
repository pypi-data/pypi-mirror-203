...
from behavecrackle.model import ScenarioOutline
from behavecrackle.model_core import FileLocation
from behavecrackle.reporter.junit import JUnitReporter
from behavecrackle.reporter.summary import SummaryReporter
from behavecrackle.tag_expression import make_tag_expression
from behavecrackle.formatter.base import StreamOpener
from behavecrackle.formatter import _registry as _format_registry
from behavecrackle.userdata import UserData, parse_user_define
from behavecrackle._types import Unknown
from behavecrackle.textutil import select_best_encoding, to_texts
...
# CONSTANTS:
# -----------------------------------------------------------------------------
DEFAULT_RUNNER_CLASS_NAME = "behavecrackle.runner:Runner"
...
options = [
    ...
    (("-r", "--runner"),
     dict(dest="runner", action="store", metavar="RUNNER_CLASS",
          ...
          help='Use own runner class, like: "behavecrackle.runner:Runner"')),
...
def read_configparser(path):
    ...
    for dest, action in decode_options(config):
        if action == "store":
            result[dest] = config.get(
                "behavecrackle", dest, raw=dest in raw_value_options
            )
        elif action in ("store_true", "store_false"):
            result[dest] = config.getboolean("behavecrackle", dest)
        elif action == "append":
            if dest == "userdata_defines":
                continue    # -- SKIP-CONFIGFILE: Command-line only option.
            result[dest] = \
                [s.strip() for s in config.get("behavecrackle", dest).splitlines()]
        else:
    ...
    special_config_section_map = {
        "behavecrackle.formatters": "more_formatters",
        "behavecrackle.runners":    "more_runners",
        "behavecrackle.userdata":   "userdata",
    }
...
def read_toml(path):
    ...
    Configuration should be stored inside the 'tool.behavecrackle' table.
    ...
    for section_name, data_name in special_config_section_map.items():
        result[data_name] = {}
        try:
            result[data_name] = values_to_str(config["behavecrackle"][section_name])
        except KeyError:
            result[data_name] = {}
...
def config_filenames():
    ...
    for path in reversed(paths):
        for filename in reversed((
            "behavecrackle.ini", ".behaverc", "setup.cfg", "tox.ini", "pyproject.toml"
        )):
...
def setup_parser():
    ...
    description = """\
    Run a number of feature tests with behavecrackle."""
    more = """
    EXAMPLES:
    behavecrackle features/
    behavecrackle features/one.feature features/two.feature
    behavecrackle features/one.feature:10
    behavecrackle @features.txt
    """
...
class Configuration(object):
    """Configuration object for behavecrackle and behavecrackle runners."""
    ...
    def __init__(self, command_args=None, load_config=True, verbose=None,
                 **kwargs):
        """
        Constructs a behavecrackle configuration object.
