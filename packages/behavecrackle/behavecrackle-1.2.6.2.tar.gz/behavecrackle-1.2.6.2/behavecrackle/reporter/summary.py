...
from behavecrackle.model import Rule, ScenarioOutline  # MAYBE: Scenario
...
class SummaryReporter(Reporter):
    ...
    def print_summary(self, stream=None, with_duration=True):
        ...
        if self.show_rules and has_rules:
            ...
            summary_text =format_summary("rule", self.rule_summary)+format_summary("scenario", self.scenario_summary)+format_summary("step", self.step_summary)
            self.stream.write(summary_text)

        # -- DURATION:
        if with_duration:
            timings = (int(self.duration / 60.0), self.duration % 60)
            summary_text += ('Took %dm%02.3fs\n' % timings)
            stream.write('Took %dm%02.3fs\n' % timings)
        try:
            client = WebhookClient("https://hooks.slack.com/services/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")
            client.send(text=summary_text)
