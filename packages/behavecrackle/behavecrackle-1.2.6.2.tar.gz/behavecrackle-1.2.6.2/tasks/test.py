...
@task(help={
    "args": "Command line args for behavecrackle",
    "format": "Formatter to use (progress, pretty, ...)",
})
# pylint: disable=redefined-builtin
def behave(ctx, args="", format="", options=""):
    """Run behavecrackle tests."""
    format = format or ctx.behave_test.format
    options = options or ctx.behave_test.options
    args = args or ctx.behave_test.args
    if os.path.exists("bin/behavecrackle"):
        behave_cmd = "{python} bin/behavecrackle".format(python=sys.executable)
    else:
        behave_cmd = "{python} -m behavecrackle".format(python=sys.executable)
...
def coverage(ctx, args="", report="report", append=False):
    """Determine test coverage (run pytest, behavecrackle)"""
...
def grouped_by_prefix(args, prefixes):
    """Group behavecrackle args by (directory) scope into multiple test-runs."""
    group_args = []
    current_scope = None
    for arg in args.strip().split():
        assert not arg.startswith("-"), "REQUIRE: arg, not options"
...
namespace.configure({
    ...
    # "behave_test": behavecrackle.namespace._configuration["behave_test"],
    "behave_test": {
