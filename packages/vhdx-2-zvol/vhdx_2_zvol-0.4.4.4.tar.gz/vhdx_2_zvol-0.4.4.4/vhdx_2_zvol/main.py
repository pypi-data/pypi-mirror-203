...
def dd_with_progress(src, dst, block_size="1M"):
    cmd = ["dd", "if=" + src, "of=" + dst, "bs=" + block_size, "status=progress"]
    p = subprocess.Popen(cmd, stderr=subprocess.PIPE, universal_newlines=True)
    progress_pattern = r"(\d+\.\d+)%"
    while True:
        output = p.stderr.readline().strip()
        if not output:
            break
    ...
    if return_code == 0:
        ...
        logger.error(f"dd exited with return code {return_code}.")
        return False
...
def convert_qcow2_to_zvol(qcow2_path, zvol, zpool):
    ...
    logging.info(f"Converting QCOW2 file '{qcow2_path}' to ZVOL dataset '{zvol_name}'")
    ...
    if qcow_size is not None:
        ...
        try:
            output = subprocess.check_output(
                ["zfs", "list", "-t", "volume", "-o", "name"]
            )
            zvol_list = output.decode("utf-8").split()
            if zvol_name in zvol_list:
                logger.info(
                    f'ZFS zvol "{zvol_name}" already exists. Skipping creation.'
                )
            else:
    ...
    try:
        if not dd_with_progress(qcow2_path, mountpoint):
            return False
        logging.info(
            f"Successfully converted QCOW2 file '{qcow2_path}' "
            + f"to ZVOL dataset '{zvol_name}'"
        )
        return True
    except subprocess.CalledProcessError as e:
        logging.error(
            f"Failed to convert QCOW2 file '{qcow2_path}' to "
            + f"ZVOL dataset '{zvol_name}'. Error: {e}"
        )
        return False
    except Exception as e:
        logging.error(
