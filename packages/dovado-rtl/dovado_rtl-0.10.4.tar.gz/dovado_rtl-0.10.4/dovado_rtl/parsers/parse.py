...
def parse(
    ...
    target_source = parser().parse(Path(project_root, input_project.target_file), num_jobs=input_project.parsing_jobs)
    ...
    if task_file.suffix == ".csv":
        ...
        parameter_sources = _parse_sources(project_root, discrete_space, parser, num_jobs=input_project.parsing_jobs)
        ...
        parameter_sources = _parse_sources(project_root, continuous_space, parser, num_jobs=input_project.parsing_jobs)
...
def _parse_sources(
        project_root: Path, space: Space, parser: type[Parser], num_jobs: int = -1
) -> dict[Path, Parsed]:
    source_to_parsed = {}
    for source in space.get_sources():
        source_to_parsed[source] = parser().parse(Path(project_root, source), num_jobs)
