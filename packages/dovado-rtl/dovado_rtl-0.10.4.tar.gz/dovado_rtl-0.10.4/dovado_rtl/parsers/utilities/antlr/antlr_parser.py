...
from dovado_rtl.parsers.utilities.parallel import Parallel


class AntlrParser(ABC, Parser):
    @abstractmethod
    def parse(self, to_parse: Path, num_jobs: int = -1) -> AntlrParsed:
        ...
    ...
    def _visit(self, parse_tree: ParseTree, num_jobs: int) -> tuple[AntlrModule]:
        visitor = self._visitor_type()
        if issubclass(type(visitor), Parallel):
            visitor.jobs = num_jobs
    ...
    def _parse(
        ...
        modules = self._visit(parse_tree, num_jobs)
