...
class WinUtil:
    ...
    def wrap_string(self, string, width, queryExpr):
        ...
        try:
            lst_wrapped = []
            dict_expr = {}
            regex_query_expr = queryExpr.replace("%", ".*")
                    ...
                    if v_full == "":
                        ...
                        for j in lst_v_full_wrap_temp:
                            ...
                            if ref == ['', '']:
                                if len(j) >= 2:
                                    new_bbname = j[0:2]
                                    if new_bbname in ('k|', 'l|', 'v|', 'd|', 'a|', 'o|', 's|', '2|', '9|', '1|'):
                                        bbname = new_bbname
                                    if bbname in ('k|', 'l|', 'v|', 'd|', 'a|', 'o|', 's|', '2|', '9|', '1|') and not(jitem[0].startswith(bbname)):
        ...
        try:
            ...
            for i in range(from_line_nr, to_line_nr):
                ...
                if len(text) > 0:
                    # noinspection PyBroadException
                    try:
                        if bbname in ("k", "v", "l", "d", "a", "o", "s", "2", "9", "1"):
