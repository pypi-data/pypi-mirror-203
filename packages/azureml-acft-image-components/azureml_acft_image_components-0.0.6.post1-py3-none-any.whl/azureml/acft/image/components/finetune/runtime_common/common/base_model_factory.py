...
from azureml.acft.image.components.finetune.runtime_common.common.exceptions import AutoMLVisionValidationException
    ...
    def download_model_weights(self, model_name: Optional[str] = None) -> str:
        ...
        if model_name not in self._pre_trained_model_names_dict:
            raise AutoMLVisionValidationException('The provided model_name is not supported.',
                                                  has_pii=False)
        from azureml.acft.image.components.finetune.runtime_common.common.pretrained_model_utilities import \
