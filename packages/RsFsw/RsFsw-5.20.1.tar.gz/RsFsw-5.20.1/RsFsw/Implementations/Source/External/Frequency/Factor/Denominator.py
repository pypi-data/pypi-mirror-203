...
class DenominatorCls:
	...
	def set(self, value: float, externalGen=repcap.ExternalGen.Nr1) -> None:
		...
		This command defines the denominator of the factor with which the analyzer frequency is multiplied to obtain the transmit
		frequency of the selected generator. This command is only valid if External Generator Control (R&S FSW-B10) is installed
		and active. Select the multiplication factor such that the frequency range of the generator is not exceeded if the
