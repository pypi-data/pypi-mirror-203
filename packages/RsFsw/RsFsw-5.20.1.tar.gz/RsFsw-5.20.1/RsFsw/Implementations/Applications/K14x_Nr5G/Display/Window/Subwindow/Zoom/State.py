...
class StateCls:
	...
	def set(self, state: bool, window=repcap.Window.Default, subWindow=repcap.SubWindow.Default) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.display.window.subwindow.zoom.state.set(state = False, window = repcap.Window.Default, subWindow = repcap.SubWindow.Default) \n
	...
	def get(self, window=repcap.Window.Default, subWindow=repcap.SubWindow.Default) -> bool:
		"""SCPI: DISPlay[:WINDow<n>][:SUBWindow<w>]:ZOOM[:STATe] \n
		Snippet: value: bool = driver.applications.k14Xnr5G.display.window.subwindow.zoom.state.get(window = repcap.Window.Default, subWindow = repcap.SubWindow.Default) \n
		This command turns the zoom on and off. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:param subWindow: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Subwindow')
