...
class StateCls:
	...
	def set(self, state: bool, powerMeter=repcap.PowerMeter.Default) -> None:
		...
		Snippet: driver.applications.k18AmplifierEt.system.communicate.rdevice.pmeter.configure.auto.state.set(state = False, powerMeter = repcap.PowerMeter.Default) \n
	...
	def get(self, powerMeter=repcap.PowerMeter.Default) -> bool:
		"""SCPI: SYSTem:COMMunicate:RDEVice:PMETer<p>:CONFigure:AUTO[:STATe] \n
		Snippet: value: bool = driver.applications.k18AmplifierEt.system.communicate.rdevice.pmeter.configure.auto.state.get(powerMeter = repcap.PowerMeter.Default) \n
		This command turns automatic assignment of a power sensor to the power sensor index on and off. \n
			:param powerMeter: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Pmeter')
