...
class PtypeCls:
	...
	def set(self, pad_type: enums.PadType, inputIx=repcap.InputIx.Default) -> None:
		...
		RsFsw.InputPy.Impedance.Ptype.set command. \n
			:param pad_type: (enum or string) SRESistor | MLPad SRESistor Series-R MLPad Minimum Loss Pad
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
		"""
		param = Conversions.enum_ext_scalar_to_str(pad_type, enums.PadType)
	...
	def get(self, inputIx=repcap.InputIx.Default) -> enums.PadType:
		...
		RsFsw.InputPy.Impedance.Ptype.set command. \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
			:return: pad_type: (enum or string) SRESistor | MLPad SRESistor Series-R MLPad Minimum Loss Pad"""
		inputIx_cmd_val = self._cmd_group.get_repcap_cmd_value(inputIx, repcap.InputIx)
		response = self._core.io.query_str(f'INPut{inputIx_cmd_val}:IQ:IMPedance:PTYPe?')
		return Conversions.str_to_scalar_enum_ext(response, enums.PadType)
