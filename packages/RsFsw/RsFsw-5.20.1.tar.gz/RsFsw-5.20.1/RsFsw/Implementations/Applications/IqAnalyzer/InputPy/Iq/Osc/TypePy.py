...
class TypePyCls:
	...
	def set(self, type_py: enums.IqType, inputIx=repcap.InputIx.Default) -> None:
		...
		Defines the format of the input signal. \n
			:param type_py: (enum or string) IQ | I IQ Both components of the complex input signal (in-phase component, quadrature component) are filtered and resampled to the sample rate of the application. The input signal is down-converted with the center frequency (Low IF I) . I The input signal at the channel providing I data is resampled to the sample rate of the application. The input signal is down-converted with the center frequency (Low IF I) .
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
		"""
		param = Conversions.enum_ext_scalar_to_str(type_py, enums.IqType)
	...
	def get(self, inputIx=repcap.InputIx.Default) -> enums.IqType:
		...
		Defines the format of the input signal. \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
			:return: type_py: (enum or string) IQ | I IQ Both components of the complex input signal (in-phase component, quadrature component) are filtered and resampled to the sample rate of the application. The input signal is down-converted with the center frequency (Low IF I) . I The input signal at the channel providing I data is resampled to the sample rate of the application. The input signal is down-converted with the center frequency (Low IF I) ."""
		inputIx_cmd_val = self._cmd_group.get_repcap_cmd_value(inputIx, repcap.InputIx)
		response = self._core.io.query_str(f'INPut{inputIx_cmd_val}:IQ:OSC:TYPE?')
		return Conversions.str_to_scalar_enum_ext(response, enums.IqType)
