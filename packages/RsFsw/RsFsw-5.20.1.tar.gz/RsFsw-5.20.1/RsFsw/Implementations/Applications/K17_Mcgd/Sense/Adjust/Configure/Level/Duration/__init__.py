...
class DurationCls:
	...
	def set(self, length: float) -> None:
		...
		Snippet: driver.applications.k17Mcgd.sense.adjust.configure.level.duration.set(length = 1.0) \n
		...
		This command defines the length of the measurement if [SENSe:]ADJust:CONFigure:LEVel:DURation:MODE is set to MANual. \n
			:param length: Numeric value in seconds Range: 0.001 to 16000.0, Unit: s
		"""
	...
	def get(self) -> float:
		"""SCPI: [SENSe]:ADJust:CONFigure[:LEVel]:DURation \n
		Snippet: value: float = driver.applications.k17Mcgd.sense.adjust.configure.level.duration.get() \n
		To determine the ideal reference level, the R&S FSW performs a measurement on the current input data.
		This command defines the length of the measurement if [SENSe:]ADJust:CONFigure:LEVel:DURation:MODE is set to MANual. \n
