...
class StateCls:
	...
	def set(self, state: bool, inputIx=repcap.InputIx.Default, antenna=repcap.Antenna.Default) -> None:
		"""SCPI: INPut<ip>:EGAin<ant>:STATe \n
		Snippet: driver.applications.k10Xlte.inputPy.egain.state.set(state = False, inputIx = repcap.InputIx.Default, antenna = repcap.Antenna.Default) \n
		...
		self._core.io.write(f'INPut{inputIx_cmd_val}:EGAin{antenna_cmd_val}:STATe {param}')

	def get(self, inputIx=repcap.InputIx.Default, antenna=repcap.Antenna.Default) -> bool:
		"""SCPI: INPut<ip>:EGAin<ant>:STATe \n
		Snippet: value: bool = driver.applications.k10Xlte.inputPy.egain.state.get(inputIx = repcap.InputIx.Default, antenna = repcap.Antenna.Default) \n
		No command help available \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
			:param antenna: optional repeated capability selector. Default value: Nr1 (settable in the interface 'State')
		...
		inputIx_cmd_val = self._cmd_group.get_repcap_cmd_value(inputIx, repcap.InputIx)
		antenna_cmd_val = self._cmd_group.get_repcap_cmd_value(antenna, repcap.Antenna)
		response = self._core.io.query_str(f'INPut{inputIx_cmd_val}:EGAin{antenna_cmd_val}:STATe?')
