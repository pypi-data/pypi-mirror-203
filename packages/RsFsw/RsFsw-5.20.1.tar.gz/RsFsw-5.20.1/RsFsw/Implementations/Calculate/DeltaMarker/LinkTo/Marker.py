...
class MarkerCls:
	...
	def set(self, state: bool, window=repcap.Window.Default, deltaMarker=repcap.DeltaMarker.Default, markerDestination=repcap.MarkerDestination.Default) -> None:
		...
		In I/Q Analyzer mode, if <md> is the reference marker for the delta marker <ms>, the relative distance (delta) between
		the two markers is maintained when you move the normal marker. In other applications, the delta marker is set to the same
		horizontal position as the marker <md>, and if <md> is moved along the x-axis, <ms> follows to the same horizontal
		position. \n
