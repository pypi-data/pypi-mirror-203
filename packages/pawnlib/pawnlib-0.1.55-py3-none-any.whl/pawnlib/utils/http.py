...
class IconRpcHelper:
    ...
    def rpc_call(self, url=None, method=None, params: dict = {}, payload: dict = {}, print_error=False, raise_on_failure=False) -> dict:
        if url:
            _url = url
        else:
            _url = self.url
    ...
    def get_step_price(self, ):
        response = self.governance_call(method="getStepPrice", governance_address=const.CHAIN_SCORE_ADDRESS, sign=False)
        return response

    def _get_step_costs(self, url=None):
        response = self.governance_call(method="getStepCosts", governance_address=const.CHAIN_SCORE_ADDRESS, sign=False)
        return response
