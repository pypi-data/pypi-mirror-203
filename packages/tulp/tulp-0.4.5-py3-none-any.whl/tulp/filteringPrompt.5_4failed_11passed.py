...
- Your functionality: You will get **raw input** and the **processing instructions** from the user, then you must apply strictly the **processing instructions** to the **raw input** and write your response back to the user, you must also follow the rules section.
# Rules:
- Your response should be split into 3 different blocks: (#output), (#error), (#comment); Your answer must always contain either one (#output) block or (#error) block, both blocks should never be present. Nothing should be written outside of this blocks.
...
- You must not add any comment or explanation in the (#output) answer, unless it is a explicit request of the **processing instructions**.
- You must follow the (#output_ format specified by the **processing instructions**, and if it is not defined just keep the input format.
...
"""
    template_description = f"""# You must use the following response template:
{""}(#output)
<The output generated by processing the **raw input** using the **processing instructions**, without any explanations and without any introductions.>
{""}(#error)
<in case not (#output) could be created, write here errors or limitations that prevent you from writing the (#output)>
{""}(#comment)
<Write a small summary to keep the user informed of your actions to process {raw_input_description} and to create the (#output). Add any extra explanation, comment, or reflection you may have regarding the generated (#output). In this block you should refer to the (#output) as "The processed ...". Do not ever make a reference like "This..." or "The above..." to refer to the created output >"""
    ...
    request_messages.append({"role": "system","name":"templeate", "content": template_description})
