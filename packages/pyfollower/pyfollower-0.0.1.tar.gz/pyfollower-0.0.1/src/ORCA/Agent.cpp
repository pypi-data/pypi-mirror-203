...
namespace RVO {
	...
	{
		float pre_v_abs = RVO::abs(prefVelocity_);
		float v_abs = RVO::abs(velocity_);
		Vector2 unit_prev = prefVelocity_ / (pre_v_abs + 1e-6);
		Vector2 unit_v = velocity_ / (v_abs + 1e-6);
		if (v_abs < 0.5 && unit_prev * unit_v < 0) {
			prefVelocity_ = unit_v * pre_v_abs;
			return;
		}
		...
		for (auto nbrs : agentNeighbors_) {
			...
			score += sign_vx_sin * v_dx_cos * (1.2f / abs_dx) * v_v_inner;
		}

		float sin_theta = 0.5 * tanh(0.5 * score) + 1e-6;
		...
		prefVelocity_ = pre_v_abs * Vector2(new_x, new_y);
	}
