    ...
    def apply_prefetch(self, queryset):
        return queryset.prefetch_related(
            Prefetch(
                ...
                .filter(
                    Q(inventories__snooze_start_date__gt=now())
                    | Q(inventories__snooze_end_date__lt=now())
                )
                .distinct(),
            ),
        ).distinct()
    ...
    def get_object(self):
        ...
        try:
            product_queryset = (
                ...
                .filter(
                    Q(product_variants__inventories__snooze_start_date__gt=now())
                    | Q(product_variants__inventories__snooze_end_date__lt=now())
                )
                .distinct()
            )
