...
def draw_background(img, backgound_cfg):
    for item in backgound_cfg:
        if item['shape']=='rectangle':
            img = draw_rectangle_transparent(img, item['pt1'], item['pt2'], item['fill_color'], item['outline_color'], item['width'])
        elif item['shape']=='circle':
            img = draw_circle_transparent(img, item['pt'], item['r'], item['fill_color'], item['outline_color'], item['width'])
...
class Drawer:
    def __init__(self, cfg):
        self.w = cfg['img_w']
        self.h = cfg['img_h']
        self.font_generator = FontGenerator(font_dir=cfg['font_dir'])
        pass
