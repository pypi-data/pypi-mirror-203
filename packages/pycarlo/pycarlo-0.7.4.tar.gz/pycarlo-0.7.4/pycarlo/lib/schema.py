...
class QueryRcaType(sgqlc.types.Enum):
    ...
    * `UPDATE`None
    '''
    __schema__ = schema
    __choices__ = ('EMPTY', 'FAILED', 'NEW', 'UPDATE')
...
class PowerBIConnectionDetails(sgqlc.types.Input):
    ...
    username = sgqlc.types.Field(String, graphql_name='username')
    '''Username when auth as a master user. Required if auth_mode is
    master_user.
    '''

    password = sgqlc.types.Field(String, graphql_name='password')
    '''Password when auth as a master user. Required if auth_mode is
    master_user.
    '''
...
class SensitivityInput(sgqlc.types.Input):
    ...
    '''Minimum delay from last update (in sec) - Deprecated'''
...
class GithubPullRequest(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('url', 'title', 'issue_number', 'author', 'repository_name', 'repository_owner', 'merged_at', 'number_of_files_changed', 'files_changed')
    url = sgqlc.types.Field(String, graphql_name='url')
    '''PR URL'''
...
class GithubPullRequestFile(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('path', 'change_type')
...
class Query(sgqlc.types.Type):
    ...
    * `max_rows` (`Int`): Maximum number of rows returned (default:
    ...
    perform_field_health_sampling = sgqlc.types.Field(CustomSQLOutputSample, graphql_name='performFieldHealthSampling', args=sgqlc.types.ArgDict((
        ('field', sgqlc.types.Arg(sgqlc.types.non_null(String), graphql_name='field', default=None)),
    ...
    * `query_type` (`String`): source (reads on the table) or
    ...
    get_lineage_node_properties_v0 = sgqlc.types.Field(sgqlc.types.list_of(NodeProperties), graphql_name='getLineageNodePropertiesV0', args=sgqlc.types.ArgDict((
        ('dw_id', sgqlc.types.Arg(UUID, graphql_name='dwId', default=None)),
        ('node_ids', sgqlc.types.Arg(sgqlc.types.list_of(String), graphql_name='nodeIds', default=None)),
        ('mcons', sgqlc.types.Arg(sgqlc.types.list_of(String), graphql_name='mcons', default=None)),
))
...
class QueryRca(sgqlc.types.Type):
    '''Query RCA result'''
    __schema__ = schema
    __field_names__ = ('table_mcon', 'group_id', 'timestamp', 'type')
    table_mcon = sgqlc.types.Field(String, graphql_name='tableMcon')
...
class DbtRun(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('created_time', 'updated_time', 'uuid', 'account_id', 'dbt_project', 'job', 'dbt_run_id', 'run_logs', 'generated_at', 'started_at', 'command', 'dbt_run_steps')
    created_time = sgqlc.types.Field(sgqlc.types.non_null(DateTime), graphql_name='createdTime')
...
class WarehouseTableHealth(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('table_id', 'full_table_id', 'warehouse', 'discovered_time', 'friendly_name', 'description', 'location', 'project_name', 'dataset', 'table_type', 'is_encrypted', 'created_time', 'last_modified', 'view_query', 'labels', 'path', 'priority', 'tracked', 'status', 'freshness_anomaly', 'size_anomaly', 'freshness_size_anomaly', 'metric_anomaly', 'dynamic_table', 'is_deleted', 'last_observed', 'is_excluded', 'data_provider', 'mcon', 'is_important', 'importance_score', 'tags', 'categories_with_monitors', 'incidents')
    table_id = sgqlc.types.Field(String, graphql_name='tableId')
