...
class Process(object):
	...
	def read_string(self, lp_base_address: int, length: int) -> Any:
		"""
		Read data from the process's memory.
	...
	def read_byte(self, lp_base_address: int, length: int = 1) -> List[hex]:
		"""
		Read data from the process's memory.
	...
	def write_string(self, lp_base_address: int, string: str) -> bool:
		"""
		Write data to the process's memory.
	...
	def write_byte(self, lp_base_address: int, write_bytes: List[hex]) -> bool:
		"""
		Write data to the process's memory.
		:param lp_base_address: The process' pointer {don't use offsets}.
		:param write_bytes: The byte(s) to be written to the process's memory
		:return: It returns True if succeed if not it raises an exception.
		"""
		try:
			for x in range(len(write_bytes)):
				write_buffer = ctypes.c_ubyte(write_bytes[x])
				lp_buffer = ctypes.byref(write_buffer)
				n_size = ctypes.sizeof(write_buffer)
				lp_number_of_bytes_written = ctypes.c_ulong(0)
