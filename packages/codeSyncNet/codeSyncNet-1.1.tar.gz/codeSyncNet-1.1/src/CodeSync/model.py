...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source_ids, target_ids=None, labels=None, stage=None, alpha=0.01, just_in_time=False, source_text_ids=None):   
        ...
        #     mask_label = torch.unsqueeze(labels, -1)
        #     mask_label = mask_label.expand(-1, shift_labels.size(1))
        #     mask_label = torch.flatten(mask_label)
        #     loss_gen = loss_fct(shift_logits.view(-1, shift_logits.size(-1))[active_loss],
        #                     shift_labels.view(-1)[active_loss])
        #     loss_gen = loss_gen*(mask_label[active_loss])
        #     loss_lm = torch.sum(loss_gen)/torch.sum(mask_label[active_loss])
        # else:
        ...
        with torch.no_grad():
            ...
            # code_embeds_m =  encoder_output_m.last_hidden_state[:, 0, :]
