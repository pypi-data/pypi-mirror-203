        ...
        def __call__(self, cls, kwargs):
            try:
                if cls not in cls._instances:
                    self.logger.log(msg="No class instance found, creating a new one.", level=logging.DEBUG)
                    cls._instances[cls] = super(self.Singleton, cls).__call__(kwargs)
                elif not (cls._instances[cls].__dict__.items() >= kwargs.items()):
                    self.logger.log(msg="Class with different state found, creating a new one.", level=logging.DEBUG)
                    cls._instances[cls] = super(self.Singleton, cls).__call__(kwargs)
                return cls._instances[cls]
            except Exception as inst:
                exceptionMessage="Singleton class failure"
                self.except_block(inst=inst, exceptionMessage=exceptionMessage)
