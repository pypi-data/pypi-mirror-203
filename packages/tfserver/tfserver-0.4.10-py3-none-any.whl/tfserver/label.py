...
class Label:
    ...
    def add_class (self, class_name):
        if isinstance (self._origin, dict):
            assert class_name not in self._origin
            self._origin [class_name] = len (self._origin)
        else:
            self._origin = set (self._origin)
            self._origin.add (class_name)
        self.systemize ()

    def remove_class (self, class_name):
        assert not isinstance (self._origin, dict), 'cannot remove class to dict origin'
        self._origin = set (self._origin)
        self._origin.pop (class_name)
        self.systemize ()
