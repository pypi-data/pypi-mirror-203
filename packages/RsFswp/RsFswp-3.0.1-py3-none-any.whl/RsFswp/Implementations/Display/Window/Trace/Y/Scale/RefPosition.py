...
class RefPositionCls:
	...
	def set(self, position: float, window=repcap.Window.Default, trace=repcap.Trace.Default) -> None:
		...
		This command defines the position of the reference value on the y-axis.
			...
			:param position: Percentage of the diagram height with 100 % corresponding to the upper diagram border Unit: PCT
	...
	def get(self, window=repcap.Window.Default, trace=repcap.Trace.Default) -> float:
		...
		This command defines the position of the reference value on the y-axis.
			...
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:param trace: optional repeated capability selector. Default value: Tr1 (settable in the interface 'Trace')
			:return: position: Percentage of the diagram height with 100 % corresponding to the upper diagram border Unit: PCT"""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		trace_cmd_val = self._cmd_group.get_repcap_cmd_value(trace, repcap.Trace)
