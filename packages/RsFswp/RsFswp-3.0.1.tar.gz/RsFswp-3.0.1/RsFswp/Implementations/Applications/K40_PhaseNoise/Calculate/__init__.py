...
class CalculateCls:
	...
	def snoise(self):
		...
		if not hasattr(self, '_snoise'):
			from .Snoise import SnoiseCls
			self._snoise = SnoiseCls(self._core, self._cmd_group)
		return self._snoise
	...
	def pnLimit(self):
		...
		if not hasattr(self, '_pnLimit'):
			from .PnLimit import PnLimitCls
			self._pnLimit = PnLimitCls(self._core, self._cmd_group)
