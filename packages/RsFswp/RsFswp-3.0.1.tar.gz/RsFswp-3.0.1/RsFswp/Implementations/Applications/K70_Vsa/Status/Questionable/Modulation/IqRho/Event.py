...
class EventCls:
	...
	def get(self, window=repcap.Window.Default) -> str:
		...
		Snippet: value: str = driver.applications.k70Vsa.status.questionable.modulation.iqRho.event.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'STATus:QUEStionable:MODulation{window_cmd_val}:IQRHo:EVENt?')
