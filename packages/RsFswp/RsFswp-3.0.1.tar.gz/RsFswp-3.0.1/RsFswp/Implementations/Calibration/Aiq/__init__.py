...
class AiqCls:
	...
	def haTiming(self):
		...
		if not hasattr(self, '_haTiming'):
			from .HaTiming import HaTimingCls
			self._haTiming = HaTimingCls(self._core, self._cmd_group)
		return self._haTiming

	def clone(self) -> 'AiqCls':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = AiqCls(self._core, self._cmd_group.parent)
