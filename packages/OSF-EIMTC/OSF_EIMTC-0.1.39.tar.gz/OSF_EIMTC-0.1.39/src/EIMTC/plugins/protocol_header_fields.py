...
class ProtocolHeaderFields(NFPlugin):
    ...
    Feature Outputs:
        - udps.protocol_header_fields: utput shape of (`n_packets`, 4). Hence `n_packets` rows and 4 columns per row where each column contains the respective feature extracted from the packet. [dir, size, time, win]
