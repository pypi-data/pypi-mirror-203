...
class GradientShapleySelector(SelectorMixin, BaseEstimator):
    ...
    def single_sample_gradient_descent_mse(self, model, single_x, y_true, lr):
        """
        Perform a single gradient descent step for a linear regression model
        ...
        single_x:
            single instance
        y_true:
            true y value belonging to the passed instance
        lr:
            learning rate
        ...
        -------
        model:
            sklearn linear regression model with parameters after gradient descent step
        loss:
            loss calculated for the passed instance
        """
    ...
    def error(self, iteration: int):
        """
        Parameters
        ----------
        iteration: int
            number of iteration run so far
    ...
    def predict(self, X, y):
        """Predict the labels (1 use for training, -1 rejected) of X according to Shapley Instance Selection algorithm
