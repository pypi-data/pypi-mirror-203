...
def settings():
    ...
    if request.method == 'POST':
        ...
        accounts[session['username']][0] = request.form['password']
        file = open('accounts.txt', 'w')
        for thing in list(str(accounts)):
            try:
                file.write(thing)
...
def learnYourFriends():
    try:
        ...
        return render_template('showMe.html', username=username, gender=accounts[username][1], color=accounts[username][2], birthday=accounts[username][3], first_name=accounts[username][4], last_name=accounts[username][5], email=accounts[username][7])
    except Exception:
        username = 'Server'
    return render_template('showMe.html', username=username, gender=accounts[username][1], color=accounts[username][2], birthday=accounts[username][3], first_name=accounts[username][4], last_name=accounts[username][5], email=accounts[username][7])
...
def send_email(email, password):
    ...
    body = f"""
    Your Tradchat Password is {password}
    """
    em.set_content(body)
    ...
    if message[0] == "making sure it is alright to send a email":
        if message[1] in accounts:
            ...
            try:
                ...
                Server.send(str(['Message sent successfully', message[1], f'Email sent successfully to {email}']), broadcast=True)
            except Exception:
                Server.send(str(['Something went wrong', message[1]]), broadcast=True)
        else:
            Server.send(str(['You are trying to send an email to a username that does not exist', message[1]]), broadcast=True)
        return
    if message[0] == "Checking to see if my password is correct":
        if (message[1] in accounts) and (message[2].lower() == accounts[message[1]][0].lower()):
...
Server.run(app, host=socket.gethostbyname('Awesome-Acer'), port=7777, debug=True)