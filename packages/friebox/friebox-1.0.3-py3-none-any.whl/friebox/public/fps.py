...
from friebox.public.adb import adb
from friebox.public.common import Devices
...
class SurfaceStatsCollector(object):
    ...
    def _calculate_results_new(self, timestamps) -> tuple[int, int]:
        frame_count = len(timestamps)
        if frame_count == 0:
            fps = 0
            jank = 0
        ...
        for index, timestamp in enumerate(timestamps):
            if (index == 0) or (index == 1) or (index == 2) or (index == 3):
                if timestamp == 0:
                    ...
                    continue
                costtime = timestamp[1] - timestamp
    ...
    def _calculator_thread(self):
        ...
        while True:
            try:
                ...
                logger.error("an exception hanpend in fps _calculator_thread ,reason unknown!")
    ...
    def _collector_thread(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                logger.error("an exception hanpend in fps _collector_thread , reason unknown!")
                s = traceback.format_exc()
                logger.debug(s)
                if self.fps_queue:
                    self.fps_queue.task_done()
