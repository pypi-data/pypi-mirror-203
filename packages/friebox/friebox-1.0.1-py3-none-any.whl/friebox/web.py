...
import sys
import traceback
from friebox.view.apis import api
from friebox.view.pages import page
...
def checkPyVer():
    ...
    msg = 'python version must be 3.10+ ,your python version is {}'.format(platform.python_version())
...
def openUrl(host: str, port: int):
    ...
    webbrowser.open('http://{}:{}/?platform=Android&lan=en_US'.format(host, port), new=2)
    logger.info('Running on http://{}:{}/?platform=Android&lan=en_US (Press CTRL+C to quit)'.format(host, port))
...
def main(host=hostIP(), port=8001):
    try:
