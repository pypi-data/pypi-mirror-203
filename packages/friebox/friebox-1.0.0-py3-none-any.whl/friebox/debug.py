...
def checkPyVer():
    ...
    if int(platform.python_version().split('.')[0]) < 3:
        ...
        logger.error('please install python::3 and pip3 install -U friebox')
        sys.exit()
...
def openUrl(host: str, port: int):
    """
    Listen and open the url after friebox is started
    :param host:
    :param port:
    :return:
    """
    flag = True
    while flag:
        logger.info('start friebox server')
        flag = getServerStatus(host, port)
    webbrowser.open(f'http://{host}:{port}/?platform=Android&lan=en_US', new=2)
    logger.info(f'Running on http://{host}:{port}/?platform=Android&lan=en_US (Press CTRL+C to quit)')


def startServer(host: str, port: int):
    """
    startup the friebox service
    :param host:
    :param port:
    :return:
    """
...
def main(host=_hostIP(), port=8001):
    """
    startup friebox
    :param host: 0.0.0.0
    :param port: 默认5000端口
    ...
    try:
        ...
        logger.info('stop friebox success')                   
