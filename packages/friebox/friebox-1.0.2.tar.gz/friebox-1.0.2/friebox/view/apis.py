...
import traceback
from friebox.public.apm import CPU, MEM, Flow, FPS, Battery, GPU, Target
from friebox.public.apm_pk import CPU_PK, MEM_PK, Flow_PK, FPS_PK
from friebox.public.common import Devices, file, Method, Install, Platform
...
def setCookie():
    ...
    friebox_host = request.args.get('friebox_host')
    ...
    resp.set_cookie('friebox_host', friebox_host)
        ...
        match(platform):
            case Platform.Android:
                ...
                if len(deviceids) > 0:
                    ...
                    device_detail = d.getDeviceDetail(deviceId=deviceids[0], platform=platform)
                    result = {'status': 1, 
                              'deviceids': deviceids, 
                              'devices': devices,
                              'pkgnames': pkgnames, 
            ...
            case Platform.iOS:
                ...
                if len(deviceinfos) > 0:
                    ...
                    device_detail = d.getDeviceDetail(deviceId=deviceinfos[0].split(':')[1], platform=platform)
                    result = {'status': 1, 
...
@api.route('/device/package_names', methods=['post', 'get'])
def packageNames():
