...
class Train:
    ...
    def training(self, model: Model, train_data: DataLoader, validation_data: DataLoader, epoch=100, clip=False,
                 optimizer=torch.optim.Adam, lr=0.001, loss_fn=torch.nn.MSELoss(), training_loss_switch=True,
                 output="") -> None:
        Tra_Val = self.create_train_agent(model, epoch, optimizer, loss_fn, lr)
        training_loss, val_loss = [], []
        for cur_epoch in range(epoch):
            Tra_Val.train(train_data, clip)
            cur_train_loss = -1
            if training_loss_switch:
                _, _, cur_train_loss = Tra_Val.validation(train_data)
