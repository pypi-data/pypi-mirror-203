...
def get_gpus() -> List[GPUInfo]:
    ...
    proc = subprocess.Popen([nvidia_smi, f"--query-gpu={','.join(fields)}", "--format=csv,noheader,nounits"], preexec_fn=lambda : os.nice(19), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    try:
