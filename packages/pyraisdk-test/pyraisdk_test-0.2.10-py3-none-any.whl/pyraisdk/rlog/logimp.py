...
class AsyncEventLogger(EventLogger):
    def __init__(self, role: str = '', instance: str=''):
        ...
        self.unstruct_queue: mp.Queue[Event] = None # type: ignore
        self.struct_queue: mp.Queue[Event] = None # type: ignore
    ...
    def start(self, sys_metrics_enable: bool = False):
        with self.mutex:
            if self.backend_loop:
                return
            self.unstruct_queue = mp.Queue(maxsize=LOGGING_QUEUE_CAPACITY)
            self.struct_queue = mp.Queue(maxsize=LOGGING_QUEUE_CAPACITY)
...
class _AsyncEventLoggerBackendLoop:
    ...
    def main(self):
        ...
        '''
        # TODO: Need to refact
        # In order to put the reporter into logging process, introduced some very ugly code.
        # A refactoring is needed to sort out these concepts.
