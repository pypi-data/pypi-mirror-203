...
def mean(X:list):
    """Finds the mean of an inputted list, sum divided by size
...
def sterror(X:list, ddof=1):
    # Finds standard error
    return std(X, ddof) / (sqrt(len(X)))
...
def effectSize(A, B, ddofA=1, ddofB=1):
    ...
    """
    return (mean(A) - mean(B)) / sqrt(((len(A) - 1) * pow(std(A, ddof=ddofA), 2)) + 
            ((len(B) - 1) * pow(std(B, ddof=ddofB), 2)) / (len(A) + len(B) - 2))