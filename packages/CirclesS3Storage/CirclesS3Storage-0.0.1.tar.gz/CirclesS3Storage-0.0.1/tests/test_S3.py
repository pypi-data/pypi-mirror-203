...
from src.AWSStorage import AwsS3Storage
...
class s3_test(unittest.TestCase):
    ...
    def test_upload(self):
        functionId = self.awsS3.upload_file(
            'C:/Git/storage-python-backedn/tests/test.txt', 'test.txt', 'python\\', 1)
    ...
    def test_download(self):
        self.awsS3.download_file(
            'python\test.txt', 'C:\\Git\\storage-python-backedn\\src\\downloadTest.txt')
        assert os.path.isfile(
            'C:\Git\storage-python-backedn\src\\downloadTest.txt')
