...
class TrackWalkersId(Callback):
    ...
    def update_ids(self, inactives: bool = True):
        ...
        parent_ids = judo.copy(self.get("parent_ids", inactives=inactives))
        new_ids = judo.copy(self.get("id_walkers", inactives=True))
        parent_ids[self.swarm.state.actives] = judo.copy(new_ids[self.swarm.state.actives])
        new_ids[self.swarm.state.actives] = new_ids_all[self.swarm.state.actives]
        self.update(
