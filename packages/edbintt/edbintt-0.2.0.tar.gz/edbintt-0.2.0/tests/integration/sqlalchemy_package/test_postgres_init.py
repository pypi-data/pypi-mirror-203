from app.database.database import Database
from app.sqlalchemy_package.postgres import Postgres
from tests.integration.database_resources.base import Base
from tests.integration.database_resources.example_table_from_common_base import (
    ExampleTableFromCommonBase,
)
from tests.integration.database_resources.example_table_from_own_base import (
    ExampleTableFromOwnBase,
)


class TestPostgresInit:
    _postgres_version = "15.2"

    def test_init_with_base(self) -> None:
        with Postgres(version=self._postgres_version, base=Base) as _:
            ...

    def test_init_with_tables(self) -> None:
        with Postgres(
            version=self._postgres_version, tables=[ExampleTableFromCommonBase]
        ) as _:
            ...

    def test_init_with_common_base_and_common_tables(self) -> None:
        with Postgres(
            version=self._postgres_version,
            base=Base,
            tables=[ExampleTableFromCommonBase],
        ) as _:
            ...

    def test_init_with_common_base_and_own_tables(self) -> None:
        with Postgres(
            version=self._postgres_version, base=Base, tables=[ExampleTableFromOwnBase]
        ) as _:
            ...

    def test_init_data(self) -> None:
        with Postgres(
            version=self._postgres_version,
            tables=[ExampleTableFromOwnBase],
            data={
                ExampleTableFromOwnBase: [
                    ExampleTableFromOwnBase(column_1=1, column_2="one"),
                    ExampleTableFromOwnBase(column_1=2, column_2="two"),
                    ExampleTableFromOwnBase(column_1=3, column_2="three"),
                ]
            },
        ) as postgres:
            database = Database(database_configuration=postgres._database_configuration)

            with database.session() as session:
                rows = session.query(ExampleTableFromOwnBase).order_by(
                    ExampleTableFromOwnBase.id
                )

                expected_row_values = [
                    {"id": 1, "column_1": 1, "column_2": "one"},
                    {"id": 2, "column_1": 2, "column_2": "two"},
                    {"id": 3, "column_1": 3, "column_2": "three"},
                ]

                number_of_rows = 0
                for index, row in enumerate(rows):
                    number_of_rows += 1

                    assert row.id == expected_row_values[index]["id"]
                    assert row.column_1 == expected_row_values[index]["column_1"]
                    assert row.column_2 == expected_row_values[index]["column_2"]

                assert number_of_rows == len(expected_row_values)
