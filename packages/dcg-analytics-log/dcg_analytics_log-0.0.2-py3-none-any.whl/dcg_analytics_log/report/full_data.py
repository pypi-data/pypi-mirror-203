...
class FullDataStatistics(DataStatistics):
    ...
    def update_log_dict(self, *columns):
        ...
        *columns: Input columns as string
        ...
        for column in columns:
            for count_dict in self.count_distinct_data_column(column):
                self.log_dict.update(count_dict)
            for nulls_dict in self.count_nulls_data_column(column):
                self.log_dict.update(nulls_dict)
    
    def count_nulls_data_column(self, column):
        """ Returns number of nulls as dictionary.
    ...
    def count_distinct_data_column(self, column:str):
