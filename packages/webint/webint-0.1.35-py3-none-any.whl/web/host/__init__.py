...
class Machine(MachineBase):
    ...
    def setup_mediasoup(self):
        ...
        with self.cd(chat_dir) as clone_wd:
            ...
            # TODO XXX with clone_wd.cd("mediasoup-demo/app") as app_wd:
            # TODO XXX     app_wd.run("npm install --legacy-peer-deps")
            # XXX pass
        self.user = "root"
        ...
        with self.supervisor("mediasoup") as config:
            ...
            # XXX config["program:mediasoup_app"] = {
            # XXX     "autostart": "true",
            # XXX     "environment": (
            # XXX         f'HTTPS_CERT_FULLCHAIN="{self.certs_dir}/selfsigned-ip.crt",'
            # XXX         f'HTTPS_CERT_PRIVKEY="{self.certs_dir}/selfsigned-ip.key"'
            # XXX     ),
            # XXX     "command": "sudo npm start",
            # XXX     "directory": self.mediasoup_dir / "app",
            # XXX     "stopsignal": "INT",
            # XXX     "user": "admin",
            # XXX }
        # XXX self.nginx_conf = "/etc/nginx"
        # XXX self.onion = "WLEKRJ.onion"
        # XXX self.run(
        # XXX     f"cat > {self.nginx_conf}/application.conf",
        # XXX     stdin=str(templates.nginx_application(self.run_dir, self.onion)),
        # XXX )
        # XXX self.run("service nginx restart")
    ...
    def setup_etherpad(self):
        ...
        with self.supervisor("etherpad") as config:
            config["program:etherpad"] = {
                ...
                "command": self.etherpad_dir / "src/bin/run.sh",
