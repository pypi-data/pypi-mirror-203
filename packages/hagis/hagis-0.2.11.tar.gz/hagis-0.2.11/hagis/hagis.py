...
class Layer(Generic[T]):
    ...
    def query(self, where_clause: str = "1=1", resultRecordCount: Optional[int] = None, **kwargs: Any) -> Iterable[T]:
        if self._is_dynamic:
        ...
        if resultRecordCount is None:
            keep_querying = False
        else:
            keep_querying = True
            kwargs["resultRecordCount"] = resultRecordCount if resultRecordCount else 1

        for row in islice(self._query(where_clause, fields, keep_querying, **kwargs), resultRecordCount):
            if self._is_dynamic:
