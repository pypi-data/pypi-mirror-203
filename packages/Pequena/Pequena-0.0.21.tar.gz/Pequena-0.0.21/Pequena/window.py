...
class Api:
    ...
    def moveWindow(self, _x, _y):
        return window.move(_x, _y)

    def resizeWindow(self, _width, _height):
        return window.resize(_width, _height)

    def setWindowName(self, _name):
        return window.set_title(_name)
...
def create_window(width=800, height=600,
    ...
    global window
    api = Api()
    window = webview.create_window(title=win_name, url=build_html, js_api=api, width=width, height=height,
