...
class VMMeta(type):
    ...
    def __call__(self, vm_or_name: str | VixVM, config: ConfigParser, host: VixHost):
        vm_name = vm_or_name.name if isinstance(vm_or_name, VixVM) else vm_or_name
        if vm_name in self.cache:
            instance = self.cache[vm_name]
            return instance
        instance = super().__call__(vm_or_name, config, host)
        self.cache[vm_name] = instance
        return instance


class VM(logger.Logger, metaclass=VMMeta):
    def __init__(self, vm_or_name: str | VixVM, config: ConfigParser, host: VixHost):
        self.config = config
        self.host = host
        if isinstance(vm_or_name, VixVM):
            self.vm = vm_or_name
            self.vm_name = str(self.vm.name)
        else:
            self.vm = None
            self.vm_name = vm_or_name
        super().__init__(name=self.vm_name)
        self.load_options()
        self.open_vm()
        self._snapshot = None
        self.dry_run = False
    ...
    def open_vm(self):
        if self.vm is not None:
            self._debug(f"Skipped opening VM for {self.vm_name} as it is already opened")
            return
        try:
            self._debug(f"Opening VM for {self.vm_name}")
            self.vm = self.host.open_vm(self.vmx_path)
            self._debug(f"Opened VM for {self.vm_name}")
        except VixError as e:
            (code,) = e.args
            if code != 4000:  # VM_NOT_FOUND
                raise
            self._debug(f"Did not open VM for {self.vm_name} as it could not be found in the host")

    def load_options(self):
        if self.vm is not None:
            self._debug(f"Loading options for {self.vm_name} from VixVM attributes")
            self.root_dir = os.path.dirname(str(self.vm.vmx_path))
            self.username = None
            self.password = None
            self.is_clone = False
        ...
        if self.config.has_section(self.vm_name):
            self._debug(f"Loading options for {self.vm_name} from the config file")
            options = self.config[self.vm_name]
            self.root_dir = options["root_dir"]
            self.username = options.get("username", None)
            self.password = options.get("password", None)
            self.is_clone = options.getboolean("is_clone", fallback=False)
            self.is_linked = options.getboolean("is_linked", fallback=False)
            self.source = options.get("source", fallback=None)
            self.read_only = options.getboolean("read_only", fallback=False)
        elif self.vm is None:
            self._error(f"Could not find any options for {self.vm_name} in host or config file")
    ...
    def get_snapshot(self) -> VixSnapshot | None:
        ...
        if not self.dry_run:
            ...
            if not self.is_present_in_config:
                self._error(f"Did not find snapshot for {self.vm_name} as its options could not be found in the config file")  # fmt: skip
            if self.is_clone:
    ...
    def login(self):
        ...
        if not self.dry_run:
            ...
            if self.username is None:
                self.username = input(f"Enter username for {self.vm_name}: ")
                self.password = getpass.getpass()
            self.vm.login(self.username, self.password)
        self._debug(f"Logged in to {self.vm_name}")
    ...
    def start(self, launch_gui: bool = False):
        ...
        self._info(f"Started {self.vm_name}")
    ...
    def restart(self, from_guest: bool = True):
        ...
        self._info(f"Restarted {self.vm_name}")
    ...
    def stop(self, from_guest: bool = True):
        ...
        self._info(f"Stopped {self.vm_name}")
    ...
    def delete(self, delete_files: bool = True):
        ...
        self._info(f"Deleted {self.vm_name}")
        self.vm = None
    ...
    def modify_computer_name(self, new_name: str):
        ...
        self._info(f"Finished modifying {self.vm_name} computer name")
    ...
    def modify_config(self, updates: dict[str, str]):
        ...
        self._info(f"Finished modifying {self.vm_name} config")
    ...
    def clone(self, vm_name: str, host: VixHost | None = None):
        ...
        if not self.dry_run:
            ...
            vm.stop()
        self._info(f"Cloned {self.vm_name} to {vm_name}")
