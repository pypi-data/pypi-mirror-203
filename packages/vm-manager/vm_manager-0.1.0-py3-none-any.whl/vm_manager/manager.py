...
class VMManager(logger.Logger):
    def __init__(self, name: str | None = None):
        ...
        self.add_command("wait", help="pause execution (tip: use wait to add some delay between execution of multiple commands)", arguments=[(["seconds"], {"type": int, "metavar": "SECS", "help": "number of seconds to wait"})], callback=self.wait_callback)
    ...
    def list_callback(self):
        ...
        vms = self.select_vms("is_present_in_host & is_present_in_config")
        if len(vms) > 0:
    ...
    def parse_args(self, args: Sequence[str] | None = None):
        ...
        # Initialize VMs
        self.vms = {}
        for vm_name in self.config.sections():
            vm = VM(vm_name, self.config, self.host)
    ...
    def classify_vms(self):
        ...
        for vm_name, vm in self.vms.items():
            ...
            if self.config[vm_name].getboolean("read_only", fallback=False):
