...
from numpy import array, cumsum, exp, float64, log, mean, nan_to_num, ravel, sum as np_sum, unique, zeros_like
...
class Simulation:
    ...
    def _rec_choice_trace(self, start_node, trace):
        ...
        """
        next_node = choice(
            self.__node_dict[start_node][:, 0], p=self.__node_dict[start_node][:, 1].astype(float64)
        )
    ...
    def _get_cross_tab(self) -> DataFrame:
        ...
        -------
            cross_tab : DataFrame
                Adjacency matrix
        """
        unique_actions = list(unique(self.data_holder.data[self.data_holder.activity_column])) + [
            "start",
            "end",
        ]
        ...
        mask = self._activity_mutation_data.groupby(
            by=[self.data_holder.activity_column, self.data_holder.activity_column + "_next"]
    ...
    def _convert_result_to_df(self):
        ...
        self._result.loc[start_act_mask, "start"] = self._result[start_act_mask].start + to_timedelta(
            self._result[start_act_mask].tmp, unit="s", errors="coerce"
        )

        try:
            self._result.start = self._result.start.dt.strftime(self.data_holder.time_format)
        ...
        if (
            ...
            self._result.drop(columns=["tmp", "duration_tmp"], inplace=True)  # TODO refactor
        else:
            self._result.drop(columns=["tmp", "duration_tmp", "duration"], inplace=True)  # TODO refactor
    ...
    def _assign_end_activity_by_process(self) -> DataFrame:
        ...
        supp_data = (
            ...
            .agg({self.data_holder.activity_column: tuple, self.data_holder.duration_column: tuple})
            .reset_index()
        )
        ...
        supp_data[self.data_holder.activity_column] = (
            supp_data["act_start"] + supp_data[self.data_holder.activity_column] + supp_data["act_end"]
        )
        supp_data[self.data_holder.duration_column] = (
            supp_data["time_start"] + supp_data[self.data_holder.duration_column] + supp_data["time_end"]
        )

        supp_data = (
            supp_data[
                [self.data_holder.id_column, self.data_holder.activity_column, self.data_holder.duration_column]
            ]
            .apply(Series.explode)
            .reset_index(drop=True)
        )
        supp_data[self.data_holder.duration_column] = supp_data[self.data_holder.duration_column].fillna(0)
        supp_data[self.data_holder.activity_column + "_next"] = supp_data[self.data_holder.activity_column].shift(
            -1
        )

        return supp_data

    def _sample_approx_activities_duration(self, zeroing_additive=1e-5) -> dict[str, NDArray[float64]]:
        """
        ...
        def smooth_duration_by_activity(duration_by_activity: Series) -> NDArray[float64]:
        ...
        duration_activity_grouped = self._activity_mutation_data.groupby(self.data_holder.activity_column)[
    ...
    def _get_duration_array(self, node, size, zeroing_additive=1e-5) -> array:
        ...
        if node not in self._action_duration:
            return normal(loc=self._mean_duration, scale=self._mean_duration**0.5, size=size)

        if len(self._action_duration[node]) == 1:
            if node not in self.__time_like_nodes:
                self._action_duration[node] = normal(1, 0.1, size + 1) * self._action_duration[node]
            else:
                mean_time_scale = (
                    mean(self._action_duration[self.__time_like_nodes[node]]) / self._action_duration[node]
                )
                self._action_duration[node] = self._action_duration[self.__time_like_nodes[node]] * mean_time_scale
    ...
    def _calc_inter_start_rate(self, size: int) -> NDArray[float64]:
        ...
        """
        start_timestamp_mask = self.data_holder.data[self.data_holder.id_column] != self.data_holder.data[
            self.data_holder.id_column
        ].shift(1)
        start_timestamps_s_numpy = (
            self.data_holder.data[start_timestamp_mask][self.data_holder.start_timestamp_column].dropna().values
        )
...
if __name__ == "__main__":
    data_holder = DataHolder(
        data="data/Test_data_for_new_what-if.xlsx",
        id_column="Новые Ключ",
        activity_column="Новый этап",
        start_timestamp_column="Entry_time",
    )
    ...
    # sim.change_edges_probabilities(
    #     {"Погрузка на карьере": {"Разгрузка у клиента": 0.23, "Склад (Перевалка)": 0.51}}, edges=True
    # )

    # удаление нод
    # sim.delete_node("Точки по движению (весы, ДПС, зоны отдыха, кафе,общежитие, платный участок)")
    # # удаление ребер (side=True - только в одну сторону)
    # sim.delete_edge("Склад (Перевалка)", "Тех. Обслуживание (шиномонтаж, Скания-сервис, мойка", side="right")
    ...
    # sim.add_node(
    #     "Кафе",
    #     nodes=["Тех. Обслуживание (шиномонтаж, Скания-сервис, мойка)", "Разгрузка у клиента"],
    #     probabilities=[0.15, 0.35],
    #     mean_time=3333,
    #     side="both",
    # )
    # # добавление вершины, prob - вероятность перехода в нее
    # sim.add_edge("Склад (Перевалка)", "Кафе", prob=0.15, side="right")

    # # работа с петлями (можно писать и sim.add_edge('node1', 'node1', prob=0.5))
    # sim.delete_loop("Тех. Обслуживание (шиномонтаж, Скания-сервис, мойка)")

    # # меняет местами ноды и их среднее время (вероятности не меняет)
    # sim.swap_nodes("Склад (Перевалка)", "База Голицыно", save_probabilities=False)
