...
def get_cluster_ssh_key_fname(cluster_uuid, logger, hard_refresh=False):
    ...
    if returncode == 0 and os.path.exists(fname):
        ...
        logger.warning(f"{logging_prefix}: {ClusterStatus.FAILED}")
        if returncode != 0:
...
def cleanup_ssh_keys(logger, active_cluster_uuids):
    ...
    :param active_cluster_uuids: Set of all cluster uuids that are not in ClusterStatus.FAILED/ClusterStatus.TERMINATED state
    """
