    ...
    if bounding_box in ['face', 'yolov5s', 'rotated', 'xywh_bbox']:
        ...
        if bounding_box in ['face', 'yolov5s','rotated', 'xywh_bbox']:
            if bounding_box in ['face', 'yolov5s']:
                ...
                assert os.path.isdir(crops_dir), f"Failed to find output dir {crops_dir}"
                crops_file = os.path.join(crops_dir, "atrain_" + FILENAME_CROP_LIST)
                assert os.path.isfile(crops_file), f"Failed to find output file {crops_file}"
                atrain_crops_df = pd.read_csv(crops_file, index_col=0)
