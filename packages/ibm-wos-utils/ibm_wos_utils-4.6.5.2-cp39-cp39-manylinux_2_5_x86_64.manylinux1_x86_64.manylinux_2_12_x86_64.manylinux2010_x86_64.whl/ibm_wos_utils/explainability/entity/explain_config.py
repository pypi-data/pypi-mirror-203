...
class ExplainConfig():
    ...
    def __init__(self, input_data_type, problem_type, feature_columns, categorical_columns=[], explanation_types=[ExplanationType.LIME.value], prediction_column="prediction", probability_column="probability", training_stats=None, features_count=None, perturbations_count=None, sample_around_instance=False, discretize_continuous=True, discretizer="quartile", kernel_width=None, kernel=None, bag_of_words=True, exclude_stopwords=False, ngrams=[1], image_segment_algo=None, schema={}, feature_selection="lasso_path", single_feature_pn_default=True, enable_single_feature_pn=False, features_schema={}, service_type=None, controllable_features=None, meta_fields=None):
        """
        Arguments:
            input_data_type: 
        ...
        self.single_feature_pn_default = single_feature_pn_default
        self.enable_single_feature_pn = enable_single_feature_pn
        self.numeric_features = [
            f for f in feature_columns if f not in categorical_columns]
        self.features_indexes = {k: v for v,
                                 k in enumerate(self.feature_columns)}
    ...
    def __validate_stats(self, training_stats):
        ...
        for k in required_keys:
            if training_stats.get(k) is None:
                raise ValueError(
                    "The value of attribute {0} in training stats is invalid.".format(k))
