...
class DB2Utils:
    ...
    def generate_partition_column_query(self, schema_name, table_name, partition_column):
        ...
        partition_column_query = "ALTER TABLE \"{0}\".\"{1}\" ALTER COLUMN \"{2}\" SET GENERATED ALWAYS AS IDENTITY;".format(schema_name, table_name, partition_column)
    ...
    def generate_index_columns_query(self, schema_name, table_name, index_columns: list=[]):
        ...
        index_name = index_name_full if len(index_name_full) < 1024 else table_name[:1016] + "_index_1"


        index_query = "CREATE INDEX \"{1}\" ON \"{0}\".\"{2}\" ({3});".format(schema_name, index_name, table_name, index_str)
