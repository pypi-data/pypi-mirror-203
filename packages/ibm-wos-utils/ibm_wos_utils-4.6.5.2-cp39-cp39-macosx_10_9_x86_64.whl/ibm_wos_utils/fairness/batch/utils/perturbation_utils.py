...
class PerturbationUtils():
    ...
    @classmethod
    def _numerical_perturbation(cls, data, column_to_perturb: str, from_group: list, to_groups: list, numerical_perturb_count_per_row: int, float_decimal_place_precision: int, numerical_perturb_seed: int, spark, from_group_count: int=None, correlation_info: dict=None): # (DataFrame, dict)
        """
        Performs the categorical perturbation on the given Spark data-frame (transformations) and returns the perturbed data frame.
        :data: The Spark data frame containing the data to be perturbed.
        :column_to_perturb: The column name on which perturbation is to be performed.
    ...
    @classmethod
    def _correlated_attr_perturbation(cls, data, protected_attribute: str, to_groups: list, spark, correlation_info: dict, pert_to_groups_count: dict, float_decimal_place_precision: int=None):
        """
    ...
    def get_corr_attr_perturbed_values(cls, correlated_attr_details: dict, sample_size: int, data_type: str, float_decimal_place_precision: int=None):
        ...
        perturbed_values = random.choices(attr_values, weights=relative_weights, k=sample_size)
        ...
        if not is_categorical:
            ...
            for val in perturbed_values:
                if is_int:
                    perturbed_value = random.randint(val[0], val[1])
                else:
                    perturbed_value = random.uniform(val[0], val[1])
                    if float_decimal_place_precision is not None:
                        perturbed_value = perturbed_value.__round__(float_decimal_place_precision)
